...
## name: develop-new-module description: 在 QCOS 项目中开发新功能模块，包含数据模型、业务逻辑、API 接口和 CLI 命令的完整开发流程

# 开发新模块

> 基于 WuYueOs (QCOS) 项目架构，描述从零开发一个新功能模块的完整步骤。

## 适用场景

当需要在 QCOS 项目中开发一个包含数据模型、业务逻辑、API 接口和 CLI 命令的完整功能模块时，按以下步骤执行。
...
- 参考现有设计模式（如 DriverManager 插件加载模式）
- 支持通过配置启用/禁用组件
- 提供 registry 便于自动发现
...
- 模型继承 `BaseTable`，注册到 `db/models/__init__.py`
...
- 在 `api/schemas/__init__.py` 中注册导出
- 路由使用 `@<api>_v1.method()` 装饰器注册
- 在 `api/posiq/routes_jsonrpc/__init__.py` 中导入路由模块
- 路由复用 `job_api_v1` entrypoint 或在 `routes.py` 新增 entrypoint

### 步骤 7: 初始化集成

- 在 `TaskScheduler` (`task_manager/task_scheduler.py`) 中添加 setter 方法
- 在 `server.py` 的 `run()` 方法中按正确顺序初始化
...
- `client.py`：添加 API 调用方法，参数使用 keyword-only (`*`)
- `shell.py`：
...
- 更新 `docs/sphinx/source/user-guide/cli.rst`，补充新增 CLI 命令的使用说明
- 包含命令语法、参数说明、使用示例

**CHANGELOG：**

- 在 `CHANGELOG.md` 中新增变更记录
...
1. **pytest 不收集 `__init__.py`**：测试必须放在 `test_*.py` 文件中
1. **循环导入**：`server.py` 初始化时使用延迟导入
1. **Schema 导出**：新增 Schema 必须在 `api/schemas/__init__.py` 中导入，否则路由中 `schemas.XxxRequest` 找不到
1. **路由注册**：新增路由模块必须在 `routes_jsonrpc/__init__.py` 中 `from . import xxx`

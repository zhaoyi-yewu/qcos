...
class TestClientUser:
    ...
    def test_lock_user(self, mock_call_json_rpc):
        ...
        mock_call_json_rpc.return_value = self.return_values
        status_code, reason, text, result = client.lock_user(
            user_name="testuser",
            action="lock",
        )
        assert status_code == 200
        mock_call_json_rpc.assert_called_once_with(
            client.user_url,
            "lock_user",
            {"user_name": "testuser", "action": "lock"},
        )

    @patch.object(Client, "call_json_rpc")
    def test_unlock_user(self, mock_call_json_rpc):
        """Test unlock_user via lock_user method."""
        mock_call_json_rpc.return_value = self.return_values
        status_code, reason, text, result = client.lock_user(
            user_name="testuser",
            action="unlock",
        ...
        assert call_args["action"] == "unlock"

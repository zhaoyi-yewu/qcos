...
class TestUserRepositoryPasswordUtilities:
    ...
    def test_verify_password_with_exception(self):
        ...
        is_valid = UserRepository.verify_password(_s('password'), _s('invalid_hash'))
        assert is_valid is False
...
class TestUserRepositoryCRUD:
    ...
    def test_create_user_with_roles(self, user_repository, in_memory_db, sample_role):
    ...
    def test_get_user_by_username_not_found(self, user_repository):
        ...
        success, error, user = user_repository.get_user_by_username('nonexistent')
        assert success is False
        assert user is None

    def test_get_user_by_username_with_auto_unlock(self, user_repository, in_memory_db):
        ...
        success, error, fetched_user = user_repository.get_user_by_username('lockeduser')
        assert success is True
    ...
    def test_update_user_basic(self, user_repository, sample_user):
        ...
        success, error, updated = user_repository.update_user(sample_user.id, update_request)
        if success:
    ...
    def test_update_user_not_found(self, user_repository):
        ...
        success, error, result = user_repository.update_user(fake_id, update_request)
        assert success is False
...
class TestUserRepositoryRoleOperations:
    ...
    def test_assign_role_success(self, user_repository, sample_user, sample_role):
        """Test assigning role to user."""
        success, error = user_repository.assign_role(sample_user.id, sample_role.role_name)
    ...
    def test_update_user_roles(self, user_repository, sample_user, in_memory_db):
...
class TestUserRepositoryLoginLogOperations:
    ...
    def test_get_login_logs_with_time_filter(self, user_repository, in_memory_db):
    ...
    def test_get_login_logs_with_user_id_found(self, user_repository, sample_user, in_memory_db):
        ...
        success, error, logs = user_repository.get_login_logs(user_id=sample_user.id)
        if success:
            assert len(logs) >= 0
...
class TestUserRepositoryTokenBlacklistOperations:
    ...
    def test_add_to_blacklist_success(self, user_repository):
        ...
        success, error = user_repository.add_to_blacklist(token_jti, expires_at)
        assert success is True
...
class TestUserRepositoryExceptionHandling:
    ...
    def test_cleanup_blacklist_with_exception(self, user_repository):
        ...
        with patch.object(user_repository, 'get_all') as mock_get:
            mock_get.side_effect = Exception('DB Error')
            with patch.object(user_repository, 'delete_by_attr') as mock_delete:
                success, count = user_repository.cleanup_blacklist()
...
class TestUserRepositoryEdgeCases:
    ...
    def test_get_user_by_username_locked_not_expired(self, user_repository, in_memory_db):
        ...
        success, error, retrieved = user_repository.get_user_by_username('locked_valid_user')
        assert success is True
    ...
    def test_delete_user_by_username_not_found(self, user_repository):
        """Test deleting non-existent user by username."""
        success, error = user_repository.delete_user_by_username('nonexistent_user')
        assert success is False
    ...
    def test_get_login_logs_by_username_multiple(self, user_repository, in_memory_db):
    ...
    def test_update_user_not_found(self, user_repository):
        ...
        success, error, user = user_repository.update_user(
            fake_id,
            UpdateUserRequest(user_id=fake_id, user_name='updated')
        )
        assert success is False
        assert user is None
    ...
    def test_assign_role_with_exception(self, user_repository):
        ...
        with patch.object(user_repository, 'rollback') as mock_rollback:
            with patch('wy_qcos.db.repositories.user.select') as mock_select:
                ...
                success, error = user_repository.assign_role('user_id', 'role_name')
                assert success is False
    ...
    def test_remove_role_with_exception(self, user_repository):
        ...
        with patch.object(user_repository, 'rollback') as mock_rollback:
            with patch('wy_qcos.db.repositories.user.select') as mock_select:
                mock_select.side_effect = Exception('DB Error')
                success, error = user_repository.remove_role('user_id', 'role_name')
                assert success is False
    ...
    def test_update_user_roles_with_exception(self, user_repository):
        ...
        with patch.object(user_repository, 'rollback') as mock_rollback:
            with patch('wy_qcos.db.repositories.user.delete') as mock_delete:
                ...
                success, error = user_repository.update_user_roles('user_id', ['role1'])
                assert success is False
    ...
    def test_create_login_log_with_cleanup(self, user_repository, in_memory_db):
    ...
    def test_update_user_with_password_change(self, user_repository, sample_user):
        """Test update user with password change."""
        update_request = UpdateUserRequest(
            user_id=sample_user.id,
            password=_s('newpassword123')
        )
    ...
    def test_update_user_with_roles_change(self, user_repository, sample_user, in_memory_db):
    ...
    def test_update_user_roles_partial_failure(self, user_repository, sample_user):
    ...
    def test_assign_role_already_assigned(self, user_repository, sample_user, sample_role):
    ...
    def test_remove_role_not_assigned(self, user_repository, sample_user, sample_role):
    ...
    def test_add_to_blacklist_verify_failure(self, user_repository):
        ...
        with patch.object(user_repository, 'create') as mock_create:
            ...
            with patch.object(user_repository._db_session, 'execute') as mock_execute:
    ...
    def test_get_user_by_id_with_auto_unlock(self, user_repository, in_memory_db):
    ...
    def test_update_user_with_no_password_no_role(self, user_repository, sample_user):
    ...
    def test_update_user_password_with_no_other_changes(self, user_repository, sample_user):
        """Test update only password without other changes."""
        update_request = UpdateUserRequest(
            user_id=sample_user.id,
            password=_s('brand_new_password_123')
        )
        success, error, updated = user_repository.update_user(
            sample_user.id, update_request
        )
    ...
    def test_delete_user_by_username_with_roles(self, user_repository, in_memory_db, sample_role):
        ...
        # Now delete
        success, error = user_repository.delete_user_by_username(user.user_name)
        assert success is True

    def test_assign_role_role_not_found(self, user_repository, sample_user):
        """Test assign role when role doesn't exist."""
        success, error = user_repository.assign_role(sample_user.id, 'nonexistent_role')
        assert success is False
        assert "not found" in error.lower()

    def test_revoke_role_success(self, user_repository, sample_user, sample_role):
        """Test revoke role (alias for remove_role)."""
        # First assign
        user_repository.assign_role(sample_user.id, sample_role.role_name)
        # Then revoke
        success, error = user_repository.revoke_role(sample_user.id, sample_role.role_name)

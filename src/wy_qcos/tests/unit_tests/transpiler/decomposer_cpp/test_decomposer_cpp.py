...
from wy_qcos.transpiler.high_performance import Decomposer
...
def run_decompose(source, target):
    d = Decomposer()

    source_cpp = convert_ir_py2cpp(source)
    gate_name_list = list({op.name for op in source})

    table, _ = d.get_decompose_rules(gate_name_list, target)
    result_cpp = d.apply_decompose_rules(source_cpp, table)

    return convert_ir_cpp2py(result_cpp)
...
class TestDecomposer:
    ...
    def test_decompose_basis_only(self):
        ...
        result = run_decompose(source, target)
    ...
    def test_decompose_simple_qasm(self):
        ...
        decomposed_gates = run_decompose(gates_list, target)

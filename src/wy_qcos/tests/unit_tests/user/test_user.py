...
class TestPasswordStrengthValidation:
    """Test password strength validation scenarios."""
...
class TestConcurrentUserOperations:
    ...
    def test_sequential_user_creation(self, user_manager):
        ...
        for i in range(3):
            user = user_manager.create_user(
                f"seq_user_{i}", _s(f"pass_{i}"), ["user"], True, False, 90
            )
            users.append(user)
    ...
    def test_duplicate_user_creation_prevention(self, user_manager):
        ...
        user1 = user_manager.create_user(
            "unique_user", _s("password123"), ["user"], True, False, 90
        )
    ...
    def test_user_deletion_during_login(self, user_manager):
        ...
        # Create user
        user = user_manager.create_user(
            "delete_test_user", _s("password123"), ["user"], True, False, 90
        )
...
class TestUserAttributeValidation:
    """Test validation of user attributes."""
...
class TestUserQueryOperations:
    """Test user query and search operations."""
    ...
    def user_manager_with_users(self, user_manager_with_mocks):
        ...
        manager.create_user(
            "query_user2", _s("password2"), ["user"], True, False, 90
        )
        manager.create_user(
...
class TestUserLifecycleIntegration:
    ...
    def test_multi_user_role_management(self, user_manager):
        ...
        admin = user_manager.create_user(
            "admin_user", _s("admin_pass"), ["super_admin"], True, False, 0
        )
        viewer = user_manager.create_user(
            "viewer_user", _s("viewer_pass"), ["viewer"], True, False, 90
        )
        regular_user = user_manager.create_user(
            "regular_user", _s("user_pass"), ["user"], True, False, 90
        )
...
class TestUserBatchOperations:
    ...
    def test_bulk_user_creation(self, user_manager):
    ...
    def test_find_users_by_role_multiple_roles(self, user_manager):
        """Test finding users with multiple role assignments."""
        # Create users with overlapping roles
        user_manager.create_user(
            "multi_user1", _s("pass123"), ["user", "admin"], True, False, 90
        )
        user_manager.create_user(
            "multi_user2", _s("pass456"), ["user"], True, False, 90
        )
        user_manager.create_user(
            "multi_user3", _s("pass789"), ["admin"], True, False, 90
        )

        # Find by each role
        user_role_users = user_manager.find_users_by_role("user")
        admin_role_users = user_manager.find_users_by_role("admin")

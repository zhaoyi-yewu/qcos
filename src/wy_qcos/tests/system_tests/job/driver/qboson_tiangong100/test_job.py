...
class TestJob:
    ...
    test_job_names = [
        "test_tiangong_submit_job",
        "test_submit_job_enable_subqubo",
    ]
    ...
    def setup_class(cls):
        ...
        assert connected, (
            f"Failed to connect to tiangong mock server at "
    ...
    def test_submit_job_enable_subqubo(self):
        ...
        StLibrary.submit_job(self.admin_client, job_info)
        success, err_msg, job_results = StLibrary.wait_and_get_job_result(
            self.admin_client, job_info, self.timeout * 10, self.interval
        )

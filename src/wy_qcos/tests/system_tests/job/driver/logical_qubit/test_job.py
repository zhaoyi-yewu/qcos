...
from .logical_qubit_api_server import main
...
class TestJob:
    ...
    def setup_class(cls):
        ...
        cls.api_port = 18607
        cls.logical_qubit_process = multiprocessing.Process(
            target=main,
            daemon=True,
            kwargs={"port": cls.api_port},
        )
        cls.logical_qubit_process.start()
        ...
        assert connected, (
            f"Failed to connect to logical qubit mock server at "
    ...
    def teardown_class(cls):
        ...
        StLibrary.cleanup_test_jobs(cls.admin_client, cls.test_job_names)
        print("Stop logical qubit server")
        cls.logical_qubit_process.terminate()

    @pytest.mark.smoke
    def test_submit_job(self):
        job_info = {
            "job_id": str(Library.create_uuid(prefix=[0xF0])),
            "job_name": "test_logical_qubit_submit_job",
            ...
            "description": "description: test_logical_qubit_submit_job",
            "backend": "lq_mq02",
            "shots": 1024,

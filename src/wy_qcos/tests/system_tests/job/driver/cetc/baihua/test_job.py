...
from .baihua_api_server import main
...
class TestJob:
    ...
    def setup_class(cls):
        ...
        cls.cetc_process = multiprocessing.Process(
            target=main,
            daemon=True,
            kwargs={"port": cls.api_port},
        )
        cls.cetc_process.start()
        ...
        assert connected, (
            f"Failed to connect to CETC mock server at "
            f"{cls.api_host}:{cls.api_port}"
        )

        # Ensure the cetc device is online so the scheduler can pick up
        # jobs. When the server uses the production conf.d/ config the
        # device monitor may have set it to offline/disconnected.
        try:
            status_code, reason, _, _ = cls.admin_client.call_json_rpc(
                cls.admin_client.device_url,
                "set_device",
                body_data={"device_name": "cetc_baihua", "status": "online"},
            )
            if status_code == 200:
                logger.info("Set cetc device status to online")
            else:
                logger.warning(
                    f"Failed to set cetc device online: {status_code} {reason}"
                )
        except Exception as e:
            logger.warning(f"Could not set cetc device online: {e}")
    ...
    def teardown_class(cls):
        ...
        StLibrary.cleanup_test_jobs(cls.admin_client, cls.test_job_names)
        print("Stop CETC mock server")
        cls.cetc_process.terminate()

    @pytest.mark.smoke
    def test_submit_job(self):
        job_info = {
            "job_id": str(Library.create_uuid(prefix=[0xF0])),
            "job_name": "test_cetc_submit_job",
            ...
            "description": "description: test_cetc_submit_job",
            "backend": "cetc_baihua",
            "shots": 1024,
            ...
            "transpiler_options": {"enable_mapping": False},
            "profiling": None,
            "callbacks": None,
            "dry_run": False,
        }

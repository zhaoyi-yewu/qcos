...
from wy_qcos.transpiler.high_performance import BaseOperation, OperationType


class DriverLqMQ02Pulse(DriverPulseBase):
    """逻辑比特 MQ02 超导脉冲驱动.

    Logical Qubit MQ02 pulse driver
    ...
    def __init__(self) -> None:
        ...
        self.token = None
        self.version = "0.0.1"
        self.alias_name = "逻辑比特 MQ02 超导脉冲驱动"
        self.description = "逻辑比特 MQ02 超导脉冲驱动"
    ...
    def submit_task(self, qc, shots):
        ...
        """
        try:
            task = self.backend.run_sequence(
                qc,
                shots=shots,
            )
    ...
    def convert_results(self, results):
        ...
        cleaned_dict_result = {
            k: v for k, v in cleaned_dict_result.items() if "2" not in k
        }

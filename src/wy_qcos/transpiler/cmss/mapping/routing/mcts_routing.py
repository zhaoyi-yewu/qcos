...
class MCTree(DiGraph):
    ...
    def __init__(self, AG, DG, **args):
        ...
        logger.debug(f"select mode :{self.select_mode}")
        logger.debug(f"mode sim :{self.mode_sim}")
...
class MCTSRouting(ABC):
    ...
    def execute_routing(
        ...
        # MCT搜索过程
        logger.debug("MCTS search process start")
        start = time.time()
        ...
        logger.debug(f"MCTS search process time: {end - start}")
        ...
        logger.debug(f"number of swaps: {len(swaps)}")
        logger.debug(f"swap scheme: {swaps}")
        # 应用每个SWAP操作
        for swap in swaps:

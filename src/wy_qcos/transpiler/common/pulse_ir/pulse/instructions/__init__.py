...
:py:class:`~wy_qcos.pulse.channels.Channel` specifying where the instruction will be applied.

Every instruction has a duration, whether explicitly included as an operand or implicitly defined.
For instance, a :py:class:`~wy_qcos.pulse.instructions.ShiftPhase` instruction can be instantiated
...
:py:class:`~wy_qcos.pulse.instructions.Delay` instruction takes an explicit duration::

    Delay(duration, channel)

An instruction can be added to a :py:class:`~wy_qcos.pulse.Schedule`, which is a

...
class Kernel:
    ...
    A kernel is responsible for integrating time-series raw data into IQ
    points.
    """
    ...
    def __repr__(self):
        name_repr = "'" + self.name + "', "
        params_repr = ", ".join(
            f"{str(k)}={str(v)}" for k, v in self.params.items()
        )
...
class Discriminator:
    """Settings for a discriminator.

    A discriminator is responsible for classifying kerneled IQ points into
    0/1 state results.
    """
...
class LoRange:
    ...
    def __eq__(self, other):
        """Check whether two LO ranges are the same.

        Two LO ranges are equal when they are of the same type and have
        the same frequency bounds.
...
class LoConfig:
    ...
    def __init__(
        ...
        lo_ranges: dict[
            DriveChannel | MeasureChannel, LoRange | tuple[int, int]
        ]
        | None = None,
    ):
        ...
        Args:
            channel_los: Dictionary of mappings from configurable channels
                to LO frequencies.
            lo_ranges: Dictionary of mappings to be enforced from
                configurable channels to ``LoRange`` values.

        Raises:
            PulseError: If the channel is not configurable or the LO is out
                of range.
        ...
        for channel, lo_range in lo_ranges.items():
            self.add_lo_range(channel, lo_range)

        channel_los = channel_los if channel_los else {}
        for channel, lo_freq in channel_los.items():
            self.add_lo(channel, lo_freq)
    ...
    def add_lo(self, channel: DriveChannel | MeasureChannel, freq: float):
        ...
        if isinstance(channel, DriveChannel):
            ...
            raise PulseError(
                f"Specified channel {channel.name} cannot be configured."
            )

    def add_lo_range(
        self,
        channel: DriveChannel | MeasureChannel,
        lo_range: LoRange | tuple[int, int],
    ):
    ...
    def check_lo(
        self, channel: DriveChannel | MeasureChannel, freq: float
    ) -> bool:
        ...
        if channel in lo_ranges:
            ...
            if not lo_range.includes(freq):
                raise PulseError(
                    f"Specified LO freq {freq:f} is out of range {lo_range}"
                )
        return True
    ...
    def meas_los(self) -> dict[MeasureChannel, float]:
        """Return the mapping from measure channels to LO frequencies."""

...
def square(
    times: np.ndarray, amp: complex, freq: float, phase: float = 0
) -> np.ndarray:
    ...
    return amp * (2 * (2 * np.floor(x) - np.floor(2 * x)) + 1).astype(
        np.complex128
    )
...
def triangle(
    ...
    """
    return amp * (
        -2 * np.abs(sawtooth(times, 1, freq, phase=(phase - np.pi / 2) / 2))
        + 1
...
def _fix_gaussian_width(
...
) -> np.ndarray | tuple[np.ndarray, complex | float]:
    r"""Enforce that the supplied gaussian pulse is zeroed at a specific width.

    This is achieved by subtracting
    $\Omega_g(center \pm zeroed_width/2)$ from all samples.

    Args:
        gaussian_samples: Input gaussian samples to adjust in place.
        ...
        zeroed_width: Subtract baseline from gaussian pulses to make sure
            $\Omega_g(center \pm zeroed_width/2)=0$ is satisfied. This is
            used to avoid large discontinuities at the start of a gaussian
            pulse. If unsupplied, defaults to $2*(center + 1)$ such that
            $\Omega_g(-1)=0$ and $\Omega_g(2*(center + 1))=0$.
        rescale_amp: If True the pulse will be rescaled so that
            $\Omega_g(center)=amp$.
    ...
    zero_offset_samples = gaussian(np.array([zeroed_width / 2]), amp, 0, sigma)
    if isinstance(zero_offset_samples, tuple):
        zero_offset = zero_offset_samples[0][0]
    else:
        zero_offset = zero_offset_samples[0]
    gaussian_samples -= zero_offset
    amp_scale_factor: complex | float = 1.0
    if rescale_amp:
        amp_scale_factor = (
            amp / (amp - zero_offset) if amp - zero_offset != 0 else 1.0
        )
...
def gaussian(
    ...
    Integrated area under curve is
    $\Omega_g(amp, sigma) = amp \times np.sqrt(2\pi \sigma^2)$.

    Args:
        times: Times to output pulse for.
        amp: Pulse amplitude at `center`. If `zeroed_width` is set, pulse
            amplitude at center will be
            $amp-\Omega_g(center \pm zeroed_width/2)$ unless `rescale_amp`
            is set, in which case all samples will be rescaled such that the
            center amplitude will be `amp`.
        center: Center (mean) of pulse.
        ...
        zeroed_width: Subtract baseline from gaussian pulses to make sure
            ...
            used to avoid large discontinuities at the start of a gaussian
            pulse.
        rescale_amp: If `zeroed_width` is not `None` and
            `rescale_amp=True`, the pulse will be rescaled so that
            $\Omega_g(center)=amp$.
...
def gaussian_deriv(
    ...
    Args:
        ...
        zeroed_width: Subtract baseline of pulse to make sure
            $\Omega_g(center \pm zeroed_width/2)=0$ is satisfied. This is
            used to avoid large discontinuities at the start of a pulse.
        rescale_amp: If `zeroed_width` is not `None` and
            `rescale_amp=True`, the pulse will be rescaled so that
            $\Omega_g(center)=amp$.
    """
    ...
    gauss_deriv = (
        -x / sigma * gauss
    )  # Note that x is shifted and normalized by sigma
    if ret_gaussian:
...
def _fix_sech_width(
    ...
    Args:
        sech_samples: Input sech samples to adjust in place.
        amp: Pulse amplitude at `center`.
        center: Center (mean) of pulse.
        sigma: Standard deviation of pulse.
        zeroed_width: Subtract baseline from sech pulses to make sure
            $\Omega_g(center \pm zeroed_width/2)=0$ is satisfied. This is
            used to avoid large discontinuities at the start of a sech pulse.
            If unsupplied, defaults to $2*(center + 1)$ such that
            $\Omega_g(-1)=0$ and $\Omega_g(2*(center + 1))=0$.
        rescale_amp: If True the pulse will be rescaled so that
            $\Omega_g(center)=amp$.
        ret_scale_factor: Return amplitude scale factor.
    ...
    zero_offset_samples = sech(np.array([zeroed_width / 2]), amp, 0, sigma)
    if isinstance(zero_offset_samples, tuple):
        zero_offset = zero_offset_samples[0][0]
    else:
        zero_offset = zero_offset_samples[0]
...
def sech(
    ...
    Args:
        ...
        zeroed_width: Subtract baseline from pulse to make sure
            $\Omega_g(center \pm zeroed_width/2)=0$ is satisfied. This is
            used to avoid large discontinuities at the start and end of the
            pulse.
        rescale_amp: If `zeroed_width` is not `None` and
            `rescale_amp=True`, the pulse will be rescaled so that
            $\Omega_g(center)=amp$.
...
def gaussian_square(
    ...
    if zeroed_width:
        if zeroed_width < square_width:
            raise PulseError(
                "zeroed_width cannot be smaller than square_width."
            )
...
def drag(
    ...
    r"""Continuous Y-only correction DRAG pulse.

    For a standard nonlinear oscillator (SNO) [1].
    ...
    Args:
        ...
        beta: Y correction amplitude. For the SNO this is
            ...
            relative coupling strength between the first excited and second
            excited states, and $\Delta_2$ is the detuning between the
            respective excited states.
        zeroed_width: Subtract baseline of drag pulse to make sure
            $\Omega_g(center \pm zeroed_width/2)=0$ is satisfied. This is
            used to avoid large discontinuities at the start of a drag pulse.
        rescale_amp: If `zeroed_width` is not `None` and
            `rescale_amp=True`, the pulse will be rescaled so that
            $\Omega_g(center)=amp$.

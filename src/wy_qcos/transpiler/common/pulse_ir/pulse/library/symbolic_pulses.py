...
These pulses are described by symbolic equations for their envelopes and
...
PulseParameterValue = ParameterExpression | int | float | complex
PulseOptionalParameterValue = PulseParameterValue | None
...
def _lifted_gaussian(
    ...
    where :math:`y^*` is the value of the un-normalized Gaussian at the
    endpoints of the pulse. This sets the endpoints to :math:`0` while
    preserving the amplitude at the center,
    i.e. :math:`y` is set to :math:`1.0`.

    Args:
        t: Symbol object representing time.
        center: Symbol or expression representing the middle point of the
            samples.
    ...
    # This causes expression inconsistency after qpy round-trip serialization
    # through sympy.
...
def _is_amplitude_valid(
    ...
    Args:
        ...
        fargs: The arguments for the lambdified envelope_lam, as given by
...
def _get_expression_args(
    ...
    Args:
        ...
        params: Dictionary of parameters, which is a superset of expression
            arguments.
    ...
    for symbol in sorted(expr.free_symbols, key=lambda s: s.name):
        ...
        try:
            ...
            raise PulseError(
                f"Pulse parameter '{symbol.name}' is not defined for this "
                "instance. "
...
class LambdifiedExpression:
    ...
    When a new symbolic expression is assigned for the first time,
    :class:`.LambdifiedExpression` internally lambdifies the expression and
    stores the resulting callback in its cache. The next time it encounters the
    same expression, it returns the cached callback.

    Note that this class is a python `Descriptor`_ and is not intended to be
    called directly by end-users. This class is designed to be attached to the
    ...
    def __init__(self, attribute: str):
        ...
        Args:
            attribute: Name of the :class:`.SymbolicPulse` attribute that
                returns the target expression to evaluate.
        """
    ...
    def __get__(self, instance, owner) -> Callable:
        ...
        if expr is None:
            raise PulseError(
                f"'{self.attribute}' of '{instance.pulse_type}' is not "
                "assigned."
            )
        key = hash(expr)
            ...
            try:
                ...
                def _wrapped_lamb(*args):
                    if isinstance(args[0], np.ndarray):
                        # When args[0] is a vector ("t"), tile the remaining
                        # arguments to avoid looping over each element in t.
                        t = args[0]
                        args = np.hstack((
                            t.reshape(t.size, 1),
                ...
                # Currently symengine doesn't support the complex_double
                # version for
                # several functions such as comparison operator and piecewise.
                # If an expression contains these functions, it falls back to
                # sympy lambdify.
                # See issue details at:
                # https://github.com/symengine/symengine.py/issues/406
                import sympy
...
class SymbolicPulse(Pulse):
    r"""Parametric pulse model with symbolic expressions.

    A symbolic pulse instance is defined by an envelope and optional parameter
    constraints. Different pulse shapes are distinguished by
    :attr:`SymbolicPulse.envelope` and :attr:`SymbolicPulse.pulse_type`
    instead of dedicated subclasses.

    Symbolic expressions may be defined with either SymPy_ or Symengine_.
    Symengine is generally faster for pulse instantiation, while SymPy supports
    a wider range of symbolic operations.
    ...
    The waveform at time :math:`t` is generated by :meth:`get_waveform`
    according to
    ...
    where :math:`\Theta` is the full set of pulse parameters in
    :attr:`SymbolicPulse.parameters`, including :math:`\rm duration`.
    The envelope is sampled on a discrete time vector when
    :meth:`SymbolicPulse.get_waveform` is called, so a symbolic pulse stores
    only parameter values and waveform shape until waveform materialization.
    ...
    When a symbolic pulse is instantiated, :meth:`.validate_parameters` checks
    both configured constraint functions and maximum amplitude conditions.
    This improves safety, but it can reduce performance. Validation may be
    disabled by setting :attr:`.disable_validation` to ``True``.

    .. _symbolic_pulse_constraints:

    .. rubric:: Constraint functions

    Constraints are defined through :attr:`SymbolicPulse.constraints` and must
    evaluate to a boolean expression over the pulse parameters. Multiple
    conditions may be combined with logical expressions such as ``And`` and
    ``Or``. The pulse is valid only when these constraint expressions evaluate
    to ``True``.
    ...
    Backends often require normalized pulse amplitudes satisfying
    :math:`\max |F| \leq 1`. When ``limit_amplitude=True``, this condition is
    validated together with the symbolic constraints. Since full waveform
    evaluation can be expensive,
    :attr:`SymbolicPulse.valid_amp_conditions` allows callers to provide a
    cheaper symbolic precondition that skips waveform construction when it
    already guarantees a valid amplitude.
    ...
    .. code-block::
        ...
        from wy_qcos.transpiler.common.pulse_ir.pulse.library import (
            SymbolicPulse,
        )
    ...
    Note that :class:`SymbolicPulse` can be instantiated without providing the
    envelope and constraints. However, such an instance cannot generate
    waveforms until the envelope definition is provided.
    ...
    Likewise, you can define :attr:`SymbolicPulse.constraints` for
    ``my_pulse``. After providing the envelope definition, you can generate
    the waveform data. In practice, it is often convenient to hide this setup
    ...
    You can also provide a :class:`Parameter` object in the
    ``parameters`` dictionary, or define ``duration`` with a
    :class:`Parameter` object when you instantiate the symbolic pulse.
    A waveform cannot be generated until you assign all unbounded parameters.
    Parameters are typically assigned through the schedule that plays the
    pulse.
    ...
    :class:`~SymbolicPulse` instances can be serialized together with their
    symbolic expressions through :mod:`qiskit.qpy`. This allows custom pulse
    definitions to be saved, reloaded, and shared with collaborators.
    """
    ...
    # Lambdify caches keyed on sympy expressions.
    # Each cache returns the corresponding callable.
    ...
    def __init__(
        self,
        pulse_type: str,
        duration: PulseParameterValue,
        parameters: Mapping[str, PulseOptionalParameterValue] | None = None,
        name: str | None = None,
        ...
        Args:
            ...
            parameters: Dictionary of pulse parameters that defines the pulse
                envelope.
            name: Display name for this particular pulse envelope.
            limit_amplitude: If ``True``, then limit the absolute value of the
                amplitude of the waveform to 1. The default is ``True`` and
                the amplitude is constrained to 1.
            envelope: Pulse envelope expression.
            constraints: Pulse parameter constraint expression.
            valid_amp_conditions: Extra conditions to skip a full-waveform
                check for the amplitude limit. If this condition is not met,
                the validation routine investigates the full waveform and
                raises an error when the amplitude norm of any data point
                exceeds 1.0. If not provided, the validation always creates a
                full waveform.
    ...
    def get_waveform(self) -> Waveform:
        r"""Return a waveform sampled from the pulse formula.

        Since the returned array is a discretized time series of the continuous
        function, this method uses a midpoint sampler.
    ...
    def validate_parameters(self) -> None:
        ...
        if self._constraints is not None:
            ...
            if not bool(self._constraints_lam(*fargs)):
                ...
                raise PulseError(
                    "Assigned parameters "
                    f"{param_repr} violate following constraint: "
                    f"{const_repr}."
                )
        ...
        if self._limit_amplitude:
            if self._valid_amp_conditions is not None:
                ...
                check_full_waveform = not bool(
                    self._valid_amp_conditions_lam(*fargs)
                )
            else:
            ...
            if check_full_waveform:
                ...
                fargs = _get_expression_args(self._envelope, self.parameters)
                time_arg = fargs.pop(0)
                if not isinstance(time_arg, np.ndarray):
                    raise PulseError(
                        "Expected the first envelope argument to be time "
                        "samples."
                    )
                scalar_fargs: list[float] = []
                for arg in fargs:
                    if isinstance(arg, np.ndarray):
                        raise PulseError(
                            "Expected non-time symbolic pulse arguments to be "
                            "scalar values."
                        )
                    scalar_fargs.append(arg)

                if not _is_amplitude_valid(
                    self._envelope_lam,
                    tuple(float(val) for val in time_arg.tolist()),
                    *scalar_fargs,
                ):
                    param_repr = ", ".join(
                        f"{p}={v}" for p, v in self.parameters.items()
                    ...
                    raise PulseError(
                        "Maximum pulse amplitude norm exceeds 1.0 with "
                        f"parameters {param_repr}. This can be overruled by "
                        "setting Pulse.limit_amplitude."
                    )

    def is_parameterized(self) -> bool:
        """Return True iff the instruction is parameterized."""
        return any(
            isinstance(val, ParameterExpression)
            for val in self.parameters.values()
        )

    @property
    def parameters(self) -> dict[str, Any]:
        params: dict[str, Any] = {"duration": self.duration}
...
class ScalableSymbolicPulse(SymbolicPulse):
    ...
    Instance of :class:`ScalableSymbolicPulse` behaves the same as an instance
    of
    ...
    where :math:`\text{F}` is some function describing the rest of the
    envelope,
    and both `amp` and `angle` are real (float). Note that both `amp` and
    `angle` are
    stored in the :attr:`parameters` dictionary of the
    :class:`ScalableSymbolicPulse`
    instance.

    When two :class:`ScalableSymbolicPulse` objects are equated, instead of
    comparing
    ...
    def __init__(
        ...
        pulse_type: str,
        duration: PulseParameterValue,
        amp: PulseParameterValue,
        angle: PulseOptionalParameterValue,
        parameters: Mapping[str, PulseOptionalParameterValue] | None = None,
        name: str | None = None,
        limit_amplitude: bool | None = None,
        envelope: sym.Expr | None = None,
        ...
        Args:
            ...
            limit_amplitude: If ``True``, then limit the absolute value of the
            amplitude of the
                waveform to 1. The default is ``True`` and the amplitude is
                constrained to 1.
            envelope: Pulse envelope expression.
            constraints: Pulse parameter constraint expression.
            valid_amp_conditions: Extra conditions to skip a full-waveform
            check for the
                amplitude limit. If this condition is not met, then the
                validation routine
                will investigate the full-waveform and raise an error when the
                amplitude norm
                of any data point exceeds 1.0. If not provided, the validation
                always
        ...
        if isinstance(amp, complex):
            raise PulseError(
                "amp represents the magnitude of the complex amplitude and "
                "can't be complex"
            )
...
class _PulseType(type):
    ...
    def __instancecheck__(cls, instance):
        ...
        warnings.warn(
            "Typechecking with the symbolic pulse subclass will be "
            f"deprecated. '{cls_alias}' subclass instance is turned into "
    ...
    def __getattr__(cls, item):
        ...
        # In addition, they conventionally offer attribute-like access to the
        # pulse parameters,
        ...
        # behave like a factory by internally instantiating SymbolicPulse and
        # returning it.
        # It is not realistic to write disable=no-member across qiskit
        # packages.
        return NotImplemented


class Gaussian(metaclass=_PulseType):
    r"""A lifted and truncated Gaussian pulse envelope.

    The mean is centered at the center of the pulse (duration / 2):

    .. math::

        \begin{aligned}
        f'(x) &= \exp\Bigl(
            -\frac12 \frac{{(x - \text{duration}/2)}^2}{\text{sigma}^2}
            \Bigr)\\
        f(x) &= \text{A} \times \frac{f'(x) - f'(-1)}{1-f'(-1)},
            \quad 0 \le x < \text{duration}
        \end{aligned}

    where :math:`f'(x)` is the gaussian waveform without lifting or amplitude
    scaling, and
    ...
    def __new__(
        ...
        Args:
            ...
            angle: The angle of the complex amplitude of the Gaussian
                envelope. Default value 0.
...
class GaussianSquare(metaclass=_PulseType):
    r"""A square pulse with Gaussian-shaped risefalls.

    Both sides are lifted so that the first sample is zero.

    Exactly one of the ``risefall_sigma_ratio`` and ``width`` parameters has
    to be specified.
    ...
    .. math::
        ...
        \\begin{aligned}
        \\text{risefall} &= \\text{risefall\\_sigma\\_ratio} \\times
        \\text{sigma}\\\\
    ...
    In both cases, the lifted gaussian square pulse :math:`f'(x)` is defined
    as:
    ...
    .. math::
        ...
        \\begin{aligned}
        f'(x) &= \\begin{cases}\
            \\exp\\biggl(-\\frac12 \\frac{(x -
            \\text{risefall})^2}{\\text{sigma}^2}\\biggr)\
                & x < \\text{risefall}\\\\
            1\
                & \\text{risefall} \\le x < \\text{risefall} +
                \\text{width}\\\\
            \\exp\\biggl(-\\frac12\
                    \\frac{{\\bigl(x - (\\text{risefall} +
                    \\text{width})\\bigr)}^2}\
                          {\\text{sigma}^2}\
                    \\biggr)\
    ...
    where :math:`f'(x)` is the gaussian square waveform without lifting or
    amplitude scaling, and
    :math:`\\text{A} = \\text{amp} \\times
    ...
    def __new__(
        ...
        Args:
            duration: Pulse length in terms of the sampling period `dt`.
            amp: The magnitude of the amplitude of the Gaussian and square
            pulse.
            sigma: A measure of how wide or narrow the Gaussian risefall is;
                see the class docstring for more details.
            width: The duration of the embedded square pulse.
            angle: The angle of the complex amplitude of the pulse. Default
            value 0.
        ...
        Raises:
            PulseError: When width and risefall_sigma_ratio are both empty or
                both non-empty.
        """
        # Convert risefall_sigma_ratio into width, which is defined in the
        # OpenPulse spec.
        if width is None and risefall_sigma_ratio is None:
            raise PulseError(
                "Either the pulse width or the risefall_sigma_ratio "
...
def GaussianSquareDrag(
    ...
    r"""A square pulse with a Drag shaped rise and fall.
    ...
    Gaussian :math:`g(x, c, σ)` and lifted gaussian
    :math:`g'(x, c, σ)` curves can be written as:
    ...
    From these, the lifted DRAG curve :math:`d'(x, c, σ, β)` can be written
    as
    ...
    .. math::
        ...
        \\begin{aligned}
        f'(x) &= \\begin{cases}\
            \\text{A} \\times d'(x, \\text{risefall}, \\text{sigma},
            \\text{beta})\
                & x < \\text{risefall}\\\\
            \\text{A}\
                & \\text{risefall} \\le x < \\text{risefall} +
    ...
    Args:
        ...
        sigma: A measure of how wide or narrow the DRAG risefall is; see the
    ...
    Raises:
        PulseError: When width and risefall_sigma_ratio are both empty or both
            non-empty.
    """
    # Convert risefall_sigma_ratio into width, which is defined in the
    # OpenPulse spec.
    if width is None and risefall_sigma_ratio is None:
        raise PulseError(
            "Either the pulse width or the risefall_sigma_ratio parameter "
            "must be specified."
        )
    if width is not None and risefall_sigma_ratio is not None:
        raise PulseError(
            "Either the pulse width or the risefall_sigma_ratio parameter can "
            "be specified"
            " but not both."
        )
    if width is None and risefall_sigma_ratio is not None:
        width = cast(
            ...
            duration - 2.0 * risefall_sigma_ratio * sigma,
        )
...
def gaussian_square_echo(
    ...
    r"""An echoed Gaussian square pulse with an active tone overlaid on it.

    The Gaussian Square Echo pulse is composed of three pulses. First, a
    Gaussian Square pulse :math:`f_{echo}(x)` with amplitude ``amp`` and phase
    ``angle`` plays for half duration, followed by a second Gaussian Square
    pulse :math:`-f_{echo}(x)` with opposite amplitude and the same phase for
    the rest of the duration. Third, a Gaussian Square pulse
    :math:`f_{active}(x)` with amplitude ``active_amp`` and phase
    ``active_angle`` plays for the entire duration. The Gaussian Square Echo
    pulse :math:`g_e()` can be written as:
    ...
    gate with
    a cross-resonance superconducting qubit architecture. When applying this
    pulse to
    the target qubit, the active portion can be used to cancel IX terms from
    the
    cross-resonance drive while the echo portion can reduce the impact of a
    static ZZ coupling.

    Exactly one of the ``risefall_sigma_ratio`` and ``width`` parameters has to
    be specified.
    ...
    References:
        ...
        .. |citation1| replace:: *Jurcevic, P., Javadi-Abhari, A., Bishop, L.
        S.,
            Lauer, I., Bogorin, D. F., Brink, M., Capelluto, L.,
            G{\"u}nl{\"u}k, O.,
    ...
    Raises:
        ...
        non-empty.
    """
    # Convert risefall_sigma_ratio into width which is defined in OpenPulse
    # spec
    if width is None and risefall_sigma_ratio is None:
        raise PulseError(
            "Either the pulse width or the risefall_sigma_ratio parameter "
    ...
    _gaussian_ledge_echo = _lifted_gaussian(
        _t, _sq_t0_echo, _duration / 2 - 1, _sigma
    )
    _gaussian_redge_echo = _lifted_gaussian(
        _t, _sq_t1_echo, _duration + 1, _sigma
    )
    ...
    _gaussian_redge_active = _lifted_gaussian(
        _t, _sq_t1_active, _duration + 1, _sigma
    )
    ...
    consts_expr = sym.And(
        ...
        _width >= 0,
        _duration >= _width,
        _duration / 2 >= _width_echo,
    )

    # Check validity of amplitudes
    valid_amp_conditions_expr = sym.And(
        sym.Abs(_amp) + sym.Abs(_active_amp) <= 1.0
    )
...
def GaussianDeriv(
    ...
    r"""An unnormalized Gaussian derivative pulse.
    ...
    .. math::
        ...
        f(x) = -\\text{A}\\frac{x-\\mu}{\\text{sigma}^{2}}\\exp
            \\left[-\\left(\\frac{x-\\mu}{2\\text{sigma}}\\right)^{2}\\right]
            ,  0 <= x < duration

    where :math:`\\text{A} = \\text{amp}
    \\times\\exp\\left(i\\times\\text{angle}\\right)`,
    and :math:`\\mu=\\text{duration}/2`.

    Args:
        duration: Pulse length in terms of the sampling period `dt`.
        amp: The magnitude of the amplitude of the pulse
            (the value of the corresponding Gaussian at the midpoint
            `duration`/2).
        sigma: A measure of how wide or narrow the corresponding Gaussian peak
        is in terms of `dt`;
            described mathematically in the class docstring.
        angle: The angle in radians of the complex phase factor uniformly
            scaling the pulse. Default value 0.
    ...
    # Prepare symbolic expressions
    _t, _duration, _amp, _angle, _sigma = sym.symbols(
        "t, duration, amp, angle, sigma"
    )
    envelope_expr = (
        -_amp
        * sym.exp(sym.I * _angle)
...
class Drag(metaclass=_PulseType):
    r"""The Derivative Removal by Adiabatic Gate (DRAG) pulse.

    This is a standard Gaussian pulse with an additional Gaussian derivative
    component and lifting applied.

    It can be calibrated either to reduce the phase error due to virtual
    population of the
    :math:`|2\\rangle` state during the pulse or to reduce the frequency
    spectrum of a
    standard Gaussian pulse near the
    :math:`|1\\rangle\\leftrightarrow|2\\rangle` transition,
    reducing the chance of leakage to the :math:`|2\\rangle` state.

    .. math::

        \\begin{aligned}
        g(x) &= \\exp\\Bigl(-\\frac12 \\frac{(x -
        \\text{duration}/2)^2}{\\text{sigma}^2}\\Bigr)\\\\
        g'(x) &= \\text{A}\\times\\frac{g(x)-g(-1)}{1-g(-1)}\\\\
        f(x) &=  g'(x) \\times \\Bigl(1 + 1j \\times \\text{beta} \\times\
            \\Bigl(-\\frac{x - \\text{duration}/2}{\\text{sigma}^2}\\Bigr)
            \\Bigr),
            \\quad 0 \\le x < \\text{duration}
        \\end{aligned}

    where :math:`g(x)` is a standard unlifted Gaussian waveform, :math:`g'(x)`
    is the lifted
    :class:`~wy_qcos.pulse.library.Gaussian` waveform, and
    :math:`\\text{A} = \\text{amp} \\times
    \\exp\\left(i\\times\\text{angle}\\right)`.
    ...
    References:
        ...
        .. |citation1| replace:: *Gambetta, J. M., Motzoi, F., Merkel, S. T. &
        Wilhelm, F. K.
        ...
        .. |citation2| replace:: *F. Motzoi, J. M. Gambetta, P. Rebentrost, and
        F. K. Wilhelm
    ...
    def __new__(
        ...
        Args:
            ...
            angle: The angle of the complex amplitude of the DRAG envelope.
            Default value 0.
            name: Display name for this pulse envelope.
            limit_amplitude: If ``True``, then limit the amplitude of the
                waveform to 1. The default is ``True`` and the amplitude is
...
class Constant(metaclass=_PulseType):
    r"""A simple constant pulse with an amplitude value and a duration.

    .. math::

        f(x) = \\text{amp}\\times\\exp\\left(i\\text{angle}\\right)    ,  0 <=
        x < duration
        f(x) = 0      ,  elsewhere
    """
    ...
    def __new__(
        ...
        Args:
            ...
            angle: The angle of the complex amplitude of the square envelope.
            Default value 0.
            name: Display name for this pulse envelope.
            limit_amplitude: If ``True``, then limit the amplitude of the
                waveform to 1. The default is ``True`` and the amplitude is
        ...
        # Note this is implemented using Piecewise instead of just returning
        # amp
        # directly because otherwise the expression has no t dependence and
        # sympy's
...
def Sin(
    ...
    Args:
        duration: Pulse length in terms of the sampling period `dt`.
        amp: The magnitude of the amplitude of the sinusoidal wave. Wave range
        is [-`amp`,`amp`].
        phase: The phase of the sinusoidal wave (note that this is not
        equivalent to the angle of
            the complex amplitude)
        freq: The frequency of the sinusoidal wave, in terms of 1 over sampling
        period.
            If not provided defaults to a single cycle (i.e
            :math:'\\frac{1}{\\text{duration}}').
            The frequency is limited to the range :math:`\\left(0,0.5\\right]`
    ...
    """
    if freq is None:
        freq = cast(float | ParameterExpression | None, 1 / duration)
...
def Cos(
    ...
    Args:
        duration: Pulse length in terms of the sampling period `dt`.
        amp: The magnitude of the amplitude of the cosine wave. Wave range is
        [-`amp`,`amp`].
        phase: The phase of the cosine wave (note that this is not equivalent
        to the angle
            of the complex amplitude).
        freq: The frequency of the cosine wave, in terms of 1 over sampling
        period.
            If not provided defaults to a single cycle (i.e
            :math:'\\frac{1}{\\text{duration}}').
            The frequency is limited to the range :math:`\\left(0,0.5\\right]`
    ...
    envelope_expr = (
        _amp
        * sym.exp(sym.I * _angle)
        * sym.cos(2 * sym.pi * _freq * _t + _phase)
    )
...
def Sawtooth(
    ...
    Args:
        duration: Pulse length in terms of the sampling period `dt`.
        amp: The magnitude of the amplitude of the sawtooth wave. Wave range is
        [-`amp`,`amp`].
        phase: The phase of the sawtooth wave (note that this is not equivalent
        to the angle
            of the complex amplitude)
        freq: The frequency of the sawtooth wave, in terms of 1 over sampling
        period.
            If not provided defaults to a single cycle (i.e
            :math:'\\frac{1}{\\text{duration}}').
            The frequency is limited to the range :math:`\\left(0,0.5\\right]`
...
def Triangle(
    ...
    r"""A triangle wave pulse.

    The envelope of the pulse is given by:

    .. math::

        f(x) = \\text{A}\\left[\\text{sawtooth}\\left(x\\right)\\right]  ,  0
        <= x < duration

    where :math:`\\text{A} = \\text{amp}
    \\times\\exp\\left(i\\times\\text{angle}\\right)`,
    and :math:`\\text{sawtooth}\\left(x\\right)` is a sawtooth wave with the
    same frequency
    as the triangle wave, but a phase shifted by :math:`\\frac{\\pi}{2}`.

    Args:
        duration: Pulse length in terms of the sampling period `dt`.
        amp: The magnitude of the amplitude of the triangle wave. Wave range is
        [-`amp`,`amp`].
        phase: The phase of the triangle wave (note that this is not equivalent
        to the angle
            of the complex amplitude)
        freq: The frequency of the triangle wave, in terms of 1 over sampling
        period.
            If not provided defaults to a single cycle (i.e
            :math:'\\frac{1}{\\text{duration}}').
            The frequency is limited to the range :math:`\\left(0,0.5\\right]`
...
def Square(
    ...
    .. math::
        ...
        f(x) = \\text{A}\\text{sign}\\left[\\sin
            \\left(2\\pi x\\times\\text{freq}+\\text{phase}\\right)\\right]  ,
            0 <= x < duration

    where :math:`\\text{A} = \\text{amp}
    \\times\\exp\\left(i\\times\\text{angle}\\right)`,
    and :math:`\\text{sign}`
    is the sign function with the convention
    :math:`\\text{sign}\\left(0\\right)=1`.

    Args:
        duration: Pulse length in terms of the sampling period ``dt``.
        amp: The magnitude of the amplitude of the square wave. Wave range is
            :math:`\\left[-\\texttt{amp},\\texttt{amp}\\right]`.
        phase: The phase of the square wave (note that this is not equivalent
        to the angle of
            the complex amplitude).
        freq: The frequency of the square wave, in terms of 1 over sampling
        period.
            If not provided defaults to a single cycle (i.e
            :math:`\\frac{1}{\\text{duration}}`).
            The frequency is limited to the range :math:`\\left(0,0.5\\right]`
...
def Sech(
    ...
    where :math:`y^{*}` is the value of :math:`y` at the endpoints (at
    :math:`x=-1
    and :math:`x=\\text{duration}+1`). This shifts the endpoints value to zero,
    while also
    rescaling to preserve the amplitude at `:math:`\\text{duration}/2``.

    Args:
        duration: Pulse length in terms of the sampling period `dt`.
        amp: The magnitude of the amplitude of the pulse (the value at the
        midpoint `duration`/2).
        sigma: A measure of how wide or narrow the sech peak is in terms of
        `dt`;
            described mathematically in the class docstring.
        angle: The angle in radians of the complex phase factor uniformly
            scaling the pulse. Default value 0.
    ...
    if zero_ends:
        ...
        envelope_expr = (
            complex_amp
            * (envelope_expr - shift_val)
            / (complex_amp - shift_val)
        )
...
def SechDeriv(
    ...
    r"""An unnormalized sech derivative pulse.

    The sech function is centered around the halfway point of the pulse, and
    the envelope of the
    pulse is given by:
    ...
    .. math::
        ...
        f(x) = \\text{A}\\frac{d}{dx}\\left[\\text{sech}
            \\left(\\frac{x-\\mu}{\\text{sigma}}\\right)\\right]  ,  0 <= x <
            duration

    where :math:`\\text{A} = \\text{amp}
    \\times\\exp\\left(i\\times\\text{angle}\\right)`,
    :math:`\\mu=\\text{duration}/2`, and :math:`d/dx` is a derivative with
    respect to `x`.

    Args:
        duration: Pulse length in terms of the sampling period `dt`.
        amp: The magnitude of the amplitude of the pulse (the value of the
        corresponding sech
            function at the midpoint `duration`/2).
        sigma: A measure of how wide or narrow the corresponding sech peak is,
        in terms of `dt`;
            described mathematically in the class docstring.
        angle: The angle in radians of the complex phase factor uniformly
            scaling the pulse. Default value 0.

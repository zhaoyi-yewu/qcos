...
Schedules are Pulse programs. They describe instruction sequences for the
control hardware. The Schedule is one of the most fundamental objects to
this pulse-level programming module. A ``Schedule`` is a representation of
a *program* in Pulse. Each schedule tracks the time of each instruction
occuring in parallel over multiple signal *channels*.
...
from wy_qcos.transpiler.common.pulse_ir.pulse.reference_manager import (
    ReferenceManager,
)
from wy_qcos.transpiler.common.pulse_ir.utils.multiprocessing import (
    is_main_process,
)
from wy_qcos.transpiler.common.pulse_ir.utils import deprecate_arg
...
"""An interval type is a tuple of a start time (inclusive) and an end time
(exclusive)."""
...
class Schedule:
    """A quantum program schedule with exact time constraints.

    over all input signal *channels* and supporting special syntaxes for
    building.
    ...
    A :obj:`.PulseError` is immediately raised when the overlap constraint is
    violated.

    In the schedule representation, we cannot parametrize the duration of
    instructions.
    Thus, we need to create a new schedule object for each duration.
    To parametrize an instruction's duration, the
    :class:`~wy_qcos.pulse.ScheduleBlock`
    ...
    def __init__(
        ...
        *schedules: ScheduleComponent | tuple[int, ScheduleComponent],
        ...
        Args:
            *schedules: Child Schedules of this parent Schedule. May either be
            passed as
                        the list of schedules, or a list of ``(start_time,
                        schedule)`` pairs.
            name: Name of this schedule. Defaults to an autogenerated string if
            not provided.
            metadata: Arbitrary key value metadata to associate with the
            schedule. This gets
                stored as free-form data in a dict in the
                :attr:`~wy_qcos.pulse.Schedule.metadata` attribute. It will not
                be directly
        ...
        for sched_pair in schedules:
            if isinstance(sched_pair, tuple):
    ...
    def initialize_from(
        ...
        Args:
            ...
            name: Name of new schedule. Name of ``schedule`` is used by
            default.
        ...
        Raises:
            PulseError: When `other_program` does not provide necessary
            information.
        """
        try:
    ...
    def metadata(self) -> dict[str, Any]:
        ...
        User provided ``dict`` of metadata for the schedule. The metadata
        contents do not affect the semantics of the program but are used to
        influence the execution of the schedule. It is expected to be passed
        between all transforms of the schedule and that providers will
        associate any schedule metadata with the results it returns from the
    ...
    def children(self) -> tuple[tuple[int, ScheduleComponent], ...]:
        ...
        Notes:
            Nested schedules are returned as-is. If you want to collect only
            instructions,
    ...
    def ch_duration(self, *channels: Channel) -> int:
        """Return the end time of the last instruction.
    ...
    def ch_start_time(self, *channels: Channel) -> int:
        """Return the time of the start of the first instruction over the.
        ...
        try:
            chan_intervals = (
                self._timeslots[chan]
                for chan in channels
                if chan in self._timeslots
            )
    ...
    def _instructions(self, time: int = 0):
        ...
        Yields:
            Iterable[Tuple[int, Instruction]]: Tuple containing the time each
                :class:`~wy_qcos.pulse.Instruction`
                starts at and the flattened :class:`~wy_qcos.pulse.Instruction`
                s.
        """
    ...
    def _immutable_shift(self, time: int, name: str | None = None) -> Schedule:
        ...
        Args:
            time: Time to shift by
            name: Name of the new schedule if call was mutable. Defaults to
            name of self
        """
    ...
    def _mutable_shift(self, time: int) -> Schedule:
        ...
        for chan, ch_timeslots in self._timeslots.items():
            timeslots[chan] = [
                (ts[0] + time, ts[1] + time) for ts in ch_timeslots
            ]
    ...
    def insert(
        ...
        """Return a new schedule with ``schedule`` inserted into ``self`` at.
    ...
    def append(
        ...
        r"""Return a new schedule with ``schedule`` inserted.
    ...
    def filter(
        ...
        *filter_funcs: Callable,
        ...
        """Return a new ``Schedule`` with only the instructions from this.

        ``Schedule`` which pass though the provided filters; i.e. an
        instruction will be retained iff every function in ``filter_funcs``
        returns ``True``, the instruction occurs on a channel type contained in
        ``channels``, the instruction type is contained in
        ``instruction_types``, and the period over which the instruction
        operates is *fully* contained in one specified in ``time_ranges`` or
        ``intervals``.

        If no arguments are provided, ``self`` is returned.

        Args:
            filter_funcs: A list of Callables which take a (int,
            Union['Schedule', Instruction])
                tuple and return a bool.
            channels: For example, ``[DriveChannel(0), AcquireChannel(0)]``.
            instruction_types: For example, ``[PulseInstruction,
            AcquireInstruction]``.
            time_ranges: For example, ``[(0, 5), (6, 10)]``.
            intervals: For example, ``[(0, 5), (6, 10)]``.
            check_subroutine: Set `True` to individually filter instructions
            inside of a subroutine
                defined by the :py:class:`~wy_qcos.pulse.instructions.Call`
                instruction.
        """
        from wy_qcos.transpiler.common.pulse_ir.pulse.filters import (
            composite_filter,
            filter_instructions,
        )

        filters = composite_filter(
            channels, instruction_types, time_ranges, intervals
        )
    ...
    def exclude(
        ...
        *filter_funcs: Callable,
        ...
        """Return a ``Schedule`` with only failing instructions.
    ...
    def _add_timeslots(self, time: int, schedule: ScheduleComponent) -> None:
        """Update all time tracking within this schedule based on the given.
        ...
        Raises:
            PulseError: If timeslots overlap or an invalid start time is
            provided.
        """
        ...
        for channel in schedule.channels:
            ...
            for idx, interval in enumerate(other_timeslots[channel]):
                ...
                try:
                    ...
                    index = _find_insertion_index(
                        self._timeslots[channel], interval
                    )
                    self._timeslots[channel].insert(index, interval)
                except PulseError as ex:
                    raise PulseError(
                        f"Schedule(name='{schedule.name or ''}') cannot be "
                        "inserted into "
                        f"Schedule(name='{self.name or ''}') at "
                        f"time {time} because its instruction on channel "
                        f"{channel} scheduled from time {interval[0]} to "
                        f"{interval[1]} overlaps with an existing instruction."
                    ) from ex
    ...
    def _remove_timeslots(self, time: int, schedule: ScheduleComponent):
        ...
        Args:
            time: The time to remove the timeslots for the ``schedule``
            component.
            schedule: The schedule to insert into self.
        ...
        for channel in schedule.channels:
            ...
            for interval in other_timeslots[channel]:
                ...
                raise PulseError(
                    f"Cannot find interval ({interval[0]}, {interval[1]}) "
                    "to remove from "
    ...
    def _replace_timeslots(
        ...
        """Replace the timeslots of ``old`` if present with the timeslots of.
    ...
    def replace(
        ...
        """Return a ``Schedule`` with a replaced instruction.
        ...
        Args:
            old: Instruction to replace.
            new: Instruction to replace with.
            inplace: Replace instruction by mutably modifying this
            ``Schedule``.
        ...
        Raises:
            PulseError: If the ``Schedule`` after replacements will has a
            timing overlap.
        """
        from .parameter_manager import ParameterManager
        ...
        if inplace:
            ...
            try:
                ...
                raise PulseError(
                    f"Replacement of {old} with {new} results in "
    ...
    def assign_parameters(
        ...
        value_dict: dict[
            ...
            ParameterValueType
            | ParameterExpression
            | Sequence[ParameterValueType | ParameterExpression],
        ],
        inplace: bool = True,
    ) -> Schedule:
        ...
        Args:
            value_dict: A mapping from parameters or parameter names (parameter
            vector
            or parameter vector name) to either numeric values (list of numeric
            values)
    ...
    def __add__(self, other: ScheduleComponent) -> Schedule:
        """Return a new schedule with ``other`` inserted within ``self`` at.
    ...
    def __eq__(self, other: object) -> bool:
        ...
        Equality is checked by verifying there is an equal instruction at every
        time
        ...
        return all(
            ...
            for self_inst, other_inst in zip(
    ...
    def __repr__(self) -> str:
        ...
        instructions = ", ".join([
            repr(instr) for instr in self.instructions[:50]
        ])
...
def _require_schedule_conversion(function: Callable) -> Callable:
    ...
    This conversation is performed for backward compatibility only if all
    durations are assigned.
    """
...
class ScheduleBlock:
    ...
    The Qiskit pulse builder is a domain specific language that is developed on
    top of
    ...
    Instead of taking individual instructions with absolute execution time
    ``t0``,
    ...
    The default context is :class:`AlignLeft`, for which all instructions are
    left-justified,
    in other words, meaning they use as-soon-as-possible scheduling.

    If you need an absolute-time interval in between instructions, you can
    explicitly
    insert :class:`~wy_qcos.pulse.instructions.Delay` instructions.

    .. rubric:: Nested blocks

    A schedule block can contain other nested blocks with different alignment
    contexts.
    This enables advanced scheduling, where a subset of instructions is
    locally scheduled in a different manner.
    Note that a :class:`.Schedule` instance cannot be directly added to a
    schedule block.
    To add a :class:`.Schedule` instance, wrap it in a :class:`.Call`
    instruction.
    This is implicitly performed when a schedule is added through the
    :ref:`pulse_builder`.

    .. rubric:: Unsupported operations

    Because the schedule block representation lacks timeslots, it cannot
    perform particular :class:`.Schedule` operations such as :meth:`insert` or
    :meth:`shift` that
    ...
    Except for these operations, :class:`.ScheduleBlock` provides full
    compatibility
    with :class:`.Schedule`.

    .. rubric:: Subroutine

    The timeslots-free representation offers much greater flexibility for
    writing pulse programs.
    Because :class:`.ScheduleBlock` only cares about the ordering of the child
    blocks
    we can add an undefined pulse sequence as a subroutine of the main program.
    If your program contains the same sequence multiple times, this
    representation may
    ...
    :class:`~wy_qcos.pulse.instructions.Reference` that is defined by
    ...
    Appended reference directives are resolved when the main program is
    executed.
    Subroutines must be assigned through :meth:`assign_references` before
    execution.

    One way to reference a subroutine in a schedule is to use the pulse
    builder's :func:`~wy_qcos.pulse.builder.reference`  function to declare an
    ...
    .. code-block:: text
       ...
       ReferenceManager:
         - ('grand_child',): ScheduleBlock(Play(Constant(duration=100,
         amp=amp1,...
    ...
    As you can see the main program cannot directly assign a subroutine to the
    "grand_child" because
    this subroutine is not called within the root program, i.e. it is
    indirectly called by "child".
    However, the returned :class:`.ReferenceManager` is a dict-like object, and
    you can still
    reach to "grand_child" via the "child" program with the following chained
    dict access.
    ...
    def __init__(
        ...
        Args:
            ...
            schedule. This gets
                stored as free-form data in a dict in the
                :attr:`~wy_qcos.pulse.ScheduleBlock.metadata` attribute. It
                will not be directly
                used in the schedule.
            alignment_context (AlignmentKind): ``AlignmentKind`` instance that
            manages
        ...
        # Note that schedule block can be nested without referencing, e.g.
        # .append(child_block),
        # and parent=None indicates the root program of the current scope.
        # The nested schedule block objects should not have _reference_manager
        # and
        # should refer to the one of the root program.
        # This also means referenced program should be assigned to the root
        # program, not to child.
        ...
        for elm in self.blocks:
            if isinstance(elm, Reference):
                raise UnassignedReferenceError(
                    f"This schedule contains unassigned reference "
                    f"{elm.ref_keys} and channels are ambiguous. Please "
                    "assign the subroutine first."
                )
    ...
    def blocks(self) -> tuple[BlockComponent, ...]:
        ...
        .. note::
            ...
            schedule first, e.g. FIFO, the returned sequence is roughly
            time-ordered.
            However, in the parallel alignment context, especially in
            the as-late-as-possible scheduling, or :class:`.AlignRight`
            context,
            the actual timing of when the instructions are issued is unknown
            until
            the :class:`.ScheduleBlock` is scheduled and converted into a
            :class:`.Schedule`.
        """
        blocks = []
    ...
    def append(
        ...
        """Return a new schedule block with ``block`` appended to the context.

        block. The execution time is automatically assigned when the block is
        converted into schedule.
        ...
        if not isinstance(block, (ScheduleBlock, Instruction)):
            raise PulseError(
                f"Appended `schedule` {block.__class__.__name__} is "
                "invalid type. "
        ...
        if (
            ...
            and block.ref_keys not in self.references
        ):
            ...
            if block.is_referenced():
                if block._parent is not None:
                    ...
                    # If this is not a parent, block.references points to the
                    # parent's reference
                    # where subroutine not referred within the 'block' may
                    # exist.
                    # Move only references existing in the 'block'.
                    # See
                    # See test_appending_child_block in test_reference.
                    for ref in _get_references(block._blocks):
                        self.references[ref.ref_keys] = block.references[
                            ref.ref_keys
                        ]
                else:
                    # Avoid using dict.update and explicitly call __set_item__
                    # for validation.
                    # Reference manager of appended block is cleared because of
                    # data reduction.
    ...
    def filter(
        ...
        *filter_funcs: Callable[..., bool],
        ...
        """Return a new ``ScheduleBlock`` with only the instructions from this.

        ``ScheduleBlock`` which pass though the provided filters; i.e. an
        instruction will be retained if every function in ``filter_funcs``
        returns ``True``, the instruction occurs on a channel type contained in
        ``channels``, and the instruction type is contained in
        ``instruction_types``.
        ...
        Args:
            filter_funcs: A list of Callables which take a ``Instruction`` and
            return a bool.
            channels: For example, ``[DriveChannel(0), AcquireChannel(0)]``.
            instruction_types: For example, ``[PulseInstruction,
            AcquireInstruction]``.
        ...
        Returns:
            ``ScheduleBlock`` consisting of instructions that matches with
            filtering condition.
        """
        from wy_qcos.transpiler.common.pulse_ir.pulse.filters import (
            composite_filter,
    ...
    def exclude(
        ...
        *filter_funcs: Callable[..., bool],
        ...
        """Return a new ``ScheduleBlock`` with excluded instructions.

        *failing* at least one of the provided filters.
        This method is the complement of :py:meth:`~ScheduleBlock.filter`, so
        that::

            self.filter(args) + self.exclude(args) == self in terms of
    ...
    def replace(
        ...
        """Return a ``ScheduleBlock`` with the ``old`` component replaced with.
        ...
        for ref_key, subroutine in self.references.items():
            if ref_key in all_references:
                warnings.warn(
                    f"Reference {ref_key} conflicts with substituted "
                    f"program {new.name}. "
        ...
        # because corresponding parameters might be also used in another block
        # element.
    ...
    def is_referenced(self) -> bool:
        """Return True iff the current schedule block contains reference to.
    ...
    def assign_parameters(
        ...
        for subroutine in self._reference_manager.values():
            ...
            # So calling assign_parameters from nested block doesn't update
            # references.
            if subroutine is None:
                continue
    ...
    def assign_references(
        ...
        It is only capable of assigning a schedule block to immediate
        references
        ...
        "root::B" and the
        reference of "B" to program "A", i.e., "B::A", is not defined in the
        root namespace.
        This prevents breaking the reference "root::B::A" by the assignment of
        "root::B".
        For example, if a user could indirectly assign "root::B::A" from the
        root program,
        one can later assign another program to "root::B" that doesn't contain
        "A" within it.
        In this situation, a reference "root::B::A" would still live in
        the reference manager of the root.
        However, the subroutine "root::B::A" would no longer be used in the
        actual pulse program.
        To assign subroutine "A" to ``nested_prog`` as a nested subprogram of
        ``main_prog``,
        ...
        Args:
            subroutine_dict: A mapping from reference key to schedule block of
            the subroutine.
            inplace: Set ``True`` to override this instance with new
            subroutine.
        ...
        if not inplace:
            new_schedule = copy.deepcopy(self)
            return new_schedule.assign_references(
                subroutine_dict, inplace=True
            )

        for key, subroutine in subroutine_dict.items():
            if key not in self.references:
                unassigned_keys = ", ".join(
                    map(repr, self.references.unassigned())
                )
                raise PulseError(
    ...
    def get_parameters(self, parameter_name: str) -> list[Parameter]:
        ...
        .. code-block:: python
            ...
            from wy_qcos.transpiler.cmss import circuit
            from wy_qcos.transpiler.common.pulse_ir import pulse
    ...
    def __eq__(self, other: object) -> bool:
        ...
        Equality is checked by verifying there is an equal instruction at every
        time
        in ``other`` for every instruction in this ``ScheduleBlock``. This
        check is
        performed by converting the instruction representation into directed
        acyclic graph,
        in which execution order of every instruction is evaluated correctly
        across all channels.
        ...
        from wy_qcos.transpiler.common.pulse_ir.pulse.transforms.dag import (
            block_to_dag as dag,
        )

        if not rx.is_isomorphic_node_match(
            dag(self), dag(other), lambda x, y: x == y
        ):
            return False
...
def _common_method(*classes):
    """A function decorator to attach the function to specified classes as a.

    method.

    .. note:: For developer: A method attached through this decorator may hurt
    readability
        of the codebase, because the method may not be detected by a code
        editor.
        Thus, this decorator should be used to a limited extent, i.e. huge
        helper method.
        By using this decorator wisely, we can reduce code maintenance overhead
        without
...
@deprecate_arg(
    "show_barriers", new_alias="plot_barriers", since="1.1.0", pending=True
)
...
def draw(
    ...
    Uses matplotlib to visualize pulse waveforms on each channel.

    Returns:
        ``matplotlib.pyplot.Figure`` when matplotlib is available.
    """
    try:
        ...
        raise RuntimeError(
            "matplotlib and numpy are required for drawing. "
            "Install via: pip install matplotlib numpy"
        ) from exc

    del show_barrier

    # Convert ScheduleBlock → Schedule so we have (t0, instruction) pairs.
    program = self
    if isinstance(program, ScheduleBlock):
        from .transforms.canonicalization import block_to_schedule

        program = block_to_schedule(program)

    # Group instructions by channel.
    from .instructions.play import Play as _Play
    from .instructions.delay import Delay as _Delay
    from .instructions.phase import (
        ShiftPhase as _ShiftPhase,
        SetPhase as _SetPhase,
    )
    from .instructions.frequency import (
        ShiftFrequency as _ShiftFrequency,
        SetFrequency as _SetFrequency,
    )

    chan_instructions: dict[Channel, list] = {}
    for t0, inst in program.instructions:
        for ch in inst.channels:
            chan_instructions.setdefault(ch, []).append((t0, inst))

    # Filter disabled channels.
    if disable_channels:
        for ch in disable_channels:
            chan_instructions.pop(ch, None)

    channels = sorted(
        chan_instructions.keys(), key=lambda c: (type(c).__name__, c.index)
    )
    n_channels = len(channels)
    if n_channels == 0:
        return None

    fig, axes = (axis, None) if axis else (None, None)
    if fig is None:
    ...
    t_range = time_range if time_range else (0, total_duration)

    for idx, chan in enumerate(channels):
        ax = axes_arr[idx] if idx < len(axes_arr) else axes_arr[-1]
        insts = chan_instructions[chan]

        for t0, inst in insts:
            if isinstance(inst, _Play):
                pulse_op = inst.operands[0]
                try:
                    wf = pulse_op.get_waveform()
                    samples = np.array(wf.samples)
                except Exception:
                    samples = np.ones(inst.duration)

                times = np.arange(t0, t0 + len(samples))
                ax.fill_between(times, np.real(samples), alpha=0.4, label="I")
                ax.plot(times, np.real(samples), linewidth=0.8)
                if np.any(np.abs(np.imag(samples)) > 1e-10):
                    ax.fill_between(
                        times,
                        np.imag(samples),
                        alpha=0.3,
                    ...
                    ax.plot(
                        times, np.imag(samples), linewidth=0.8, color="orange"
                    )
                if show_waveform_info:
                    ax.annotate(
                        inst.name,
                        xy=(t0, 0),
                        fontsize=7,
            ...
            elif isinstance(inst, _Delay):
                ax.axvspan(t0, t0 + inst.duration, alpha=0.05, color="gray")

            elif (
                isinstance(inst, (_ShiftPhase, _SetPhase)) and show_framechange
            ):
                ax.axvline(
                    t0,
                    color="purple",
                ...
                label = (
                    f"Δφ={inst.operands[0]:.2f}"
                    if isinstance(inst, _ShiftPhase)
                    else f"φ={inst.operands[0]:.2f}"
                )
                ax.annotate(
                    label, xy=(t0, 0), fontsize=6, color="purple", alpha=0.8
                )

            elif (
                isinstance(inst, (_ShiftFrequency, _SetFrequency))
                and show_framechange
            ):
                ax.axvline(
                    t0, color="green", linestyle="--", linewidth=0.6, alpha=0.7
                )

        ax.set_ylabel(str(chan), fontsize=9)
        ax.set_xlim(*t_range)
        ax.axhline(0, color="black", linewidth=0.3)

    axes_arr[-1].set_xlabel(f"Time ({time_unit})")
    fig_obj = axes_arr[0].get_figure()
    fig_obj.suptitle(getattr(self, "name", "Schedule"), fontsize=12)
...
def _interval_index(intervals: list[Interval], interval: Interval) -> int:
    ...
    Args:
        ...
        interval: The interval for which the index into intervals will be
        found.
    ...
    if found_interval != interval:
        raise PulseError(
            f"The interval: {interval} does not exist in intervals: "
            f"{intervals}"
        )
    return index
...
def _locate_interval_index(
    ...
    Args:
        ...
        interval: The interval for which the index into intervals will be
        found.
        index: A running tally of the index, for recursion. The user should not
        pass a value.

    Returns:
        The index into intervals that new_interval would be inserted to
        maintain
    ...
    if interval[1] <= mid[0] and (interval != mid):
        return _locate_interval_index(
            intervals[:mid_idx], interval, index=index
        )
    else:
        return _locate_interval_index(
...
def _find_insertion_index(
    ...
    """Using binary search on start times, return the index into `intervals`.

    where the new interval belongs, or raise an error if the new interval
    overlaps with any existing ones.

    Args:
        intervals: A sorted list of non-overlapping Intervals.
        new_interval: The interval for which the index into intervals will be
        found.

    Returns:
        The index into intervals that new_interval should be inserted to
...
def _overlaps(first: Interval, second: Interval) -> bool:
    ...
    Note: first.stop may equal second.start, since Interval stop times are
    exclusive.
    """
...
def _get_timeslots(schedule: ScheduleComponent) -> TimeSlots:
    ...
    if isinstance(schedule, Instruction):
        ...
        raise PulseError(
            f"Invalid schedule type {type(schedule)} is specified."
        )
...
# These type aliases are defined at the bottom of the file, because as of
# 2022-01-18 they are
# imported into other parts of Terra.  Previously, the aliases were at the top
# of the file and used
# forwards references within themselves.  This was fine within the same file,
# but causes scoping
# issues when the aliases are imported into different scopes, in which the
# `ForwardRef` instances
# would no longer resolve.  Instead, we only use forward references in the
# annotations of _this_
# file to reference the aliases, which are guaranteed to resolve in scope, so
# the aliases can all be
# concrete.

ScheduleComponent: TypeAlias = Schedule | Instruction
"""An element that composes a pulse schedule."""

BlockComponent: TypeAlias = ScheduleBlock | Instruction

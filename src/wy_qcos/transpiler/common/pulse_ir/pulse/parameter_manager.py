...
In contrast to ``QuantumCircuit``, in pulse programs, parameter
objects can be stored in multiple places at different layers, for
example:
...
This complexity is due to the tight coupling of the program to an
underlying device Hamiltonian, i.e. the variance of physical
parameters between qubits and their couplings. If we want to define a
program that can be used with arbitrary qubits, we should be able to
parametrize every control parameter in the program.

Implementation
==============

Managing parameters in each object within a program, i.e. the
``ParameterTable`` model, makes the framework quite complicated. With
the ``ParameterManager`` class within this module, the parameter
assignment operation is performed by a visitor instance.

The visitor pattern is a way of separating data processing from the
object on which it operates. This removes the overhead of parameter
management from each piece of the program. The computational
complexity of the parameter assignment operation may be increased from
the parameter table model of ~O(1), however, usually, this calculation
occurs only once before the program is executed. Thus this doesn't
hurt user experience during pulse programming. On the contrary, it
removes parameter table object and associated logic from each object,
yielding smaller object creation cost and higher performance as the
data amount scales.

Note that we don't need to write any parameter management logic for
each object, and thus this parameter framework gives greater
scalability to the pulse module.
"""
...
from wy_qcos.transpiler.cmss.circuit.parametervector import (
    ParameterVector,
...
from wy_qcos.transpiler.common.pulse_ir.pulse import instructions, channels
...
ParameterBindValue = ParameterValueType | ParameterExpression


class NodeVisitor:
    """Walk pulse-program nodes and dispatch visitor functions.

    Though this class implementation is based on Python AST, each node
    doesn't have a dedicated node class due to the lack of an abstract
    syntax tree for pulse programs in Qiskit. Instead of parsing pulse
    programs, this visitor class finds the associated visitor function
    based on class name of the instruction node, i.e. ``Play``,
    ``Call``, etc. The ``.visit`` method recursively checks the
    superclass of a given node since some parameterized components such
    as ``DriveChannel`` may share a common superclass with other
    subclasses. In this example, we can just define
    ``visit_Channel`` instead of defining the same visitor function for
    every subclass.

    Some instructions may have special logic or data structures to
    store parameter objects, and visitor functions for these nodes
    should be individually defined.
    ...
    visitor functions. If a visitor function is not defined for a given
    node, ``generic_visit`` is called. Usually, this method is provided
    for operating on objects defined outside of the Qiskit Pulse
    module.
    """
    ...
    def _get_visitor(self, node_class):
        """Recursively investigate the superclass visitor method."""
        if node_class is object:
    ...
    def visit_ScheduleBlock(self, node: ScheduleBlock):
        """Visit ``ScheduleBlock`` and recursively overwrite context blocks.

        .. note:: ``ScheduleBlock`` can have parameters in blocks and
            its alignment.
        """
        raise NotImplementedError

    def visit_Schedule(self, node: Schedule):
        """Visit ``Schedule`` and recursively overwrite schedule children."""
        raise NotImplementedError
...
class ParameterSetter(NodeVisitor):
    ...
    This visitor is initialized with a dictionary of parameters to be
    assigned and assigns values to operands of nodes found.
    """

    def __init__(
        self, param_map: Mapping[ParameterExpression, ParameterBindValue]
    ):
    ...
    def visit_ScheduleBlock(self, node: ScheduleBlock):
        ...
        .. note:: ``ScheduleBlock`` can have parameters in blocks and
            its alignment.
        """
        node._alignment_context = self.visit_AlignmentKind(
            node.alignment_context
        )
    ...
    def visit_AlignmentKind(self, node: AlignmentKind):
        ...
        new_parameters = tuple(
            self.visit(param) for param in node._context_params
        )
    ...
    def visit_Channel(self, node: channels.Channel):
        ...
        if node.is_parameterized():
            new_index = self._assign_parameter_expression(
                cast(ParameterExpression, node.index)
            )
            ...
            if not isinstance(new_index, ParameterExpression):
                if not isinstance(new_index, int) or new_index < 0:
                    raise PulseError(
                        "Channel index must be a nonnegative integer"
                    )

            # Return a new instance to avoid overwriting timeslots before
            # they are evaluated.
    ...
    def visit_SymbolicPulse(self, node: SymbolicPulse):
        ...
        if node.is_parameterized():
            ...
            if isinstance(node.duration, ParameterExpression):
                node.duration = self._assign_parameter_expression(
                    node.duration
                )
    ...
    def generic_visit(self, node: Any):
        """Assign parameters to an object outside the Qiskit Pulse module."""
    ...
    def _assign_parameter_expression(self, param_expr: ParameterExpression):
        """Assign a parameter value to a parameter expression."""
        new_value: Any = copy(param_expr)
        updated = param_expr.parameters & self._param_map.keys()
        for param in updated:
            new_value = cast(Any, new_value).assign(
                param, self._param_map[param]
            )
        new_value = format_parameter_value(new_value)
        return new_value

    def _update_parameter_manager(self, node: Schedule | ScheduleBlock):
        """Update the parameter manager of a pulse program."""
        if not hasattr(node, "_parameter_manager"):
            raise PulseError(
                "Node type "
                f"{node.__class__.__name__} has no parameter manager."
            )
...
class ParameterGetter(NodeVisitor):
    ...
    This visitor initializes an empty parameter array, recursively
    visits nodes, and adds discovered parameters to the array.
    """
    ...
    def visit_ScheduleBlock(self, node: ScheduleBlock):
        ...
        .. note:: ``ScheduleBlock`` can have parameters in blocks and
            its alignment.
        """
        # node.parameters returns parameters of the main program with
        # subroutines. The main program manager is not aware of
        # parameters in subroutines.
        self.parameters |= node._parameter_manager.parameters

    def visit_Schedule(self, node: Schedule):
        """Visit ``Schedule`` and search schedule children for parameters."""
    ...
    def generic_visit(self, node: Any):
        """Get parameters from an object outside the Qiskit Pulse module."""
        if isinstance(node, ParameterExpression):
            self.parameters |= node.parameters


class ParameterManager:
    """Manage parameter objects associated with arbitrary pulse programs.
    ...
    Instruction data and its location are not directly associated with
    this object.
    """
    ...
    def get_parameters(self, parameter_name: str) -> list[Parameter]:
        ...
        """
        return [
            param for param in self.parameters if param.name == parameter_name
        ]
    ...
    def assign_parameters(
        ...
        value_dict: dict[
            ...
            ParameterBindValue | Sequence[ParameterBindValue],
        ],
    ) -> Any:
        """Return program data with parameters assigned according to the input.

        Args:
            pulse_program: Arbitrary pulse program associated with this
                manager instance.
            value_dict: A mapping from Parameters to either numeric
                values or another Parameter expression.
    ...
    def update_parameter_table(self, new_node: Any):
        """Update the parameter table with the given data node.
    ...
    def _unroll_param_dict(
    ...
    ) -> Mapping[Parameter, ParameterBindValue]:
        """Unroll a parameter dictionary to a map from parameter to value.

        Args:
            parameter_binds: A dictionary from parameter to value or a
                list of values.

        Returns:
            A dictionary from parameter to value.
        """
        out: dict[Parameter, ParameterBindValue] = {}
        ...
        for parameter, value in parameter_binds.items():
            if isinstance(parameter, ParameterVector):
                ...
                out.update(zip(parameter, cast(Sequence[Any], value)))
                ...
                for param in param_name_dict[parameter]:
                    ...
                    if is_vec:
                        out.update(
                            zip(
                                cast(Sequence[Any], param),
                                cast(Sequence[Any], value),
                            )
                        )
                    else:
                        out[param] = cast(ParameterBindValue, value)
            else:
                out[cast(Parameter, parameter)] = cast(
                    ParameterBindValue, value
                )
        return out

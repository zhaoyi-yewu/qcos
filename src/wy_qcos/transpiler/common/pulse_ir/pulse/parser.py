...
class PulseExpression(ast.NodeTransformer):
    ...
    def __init__(
        self, source: str | ast.Expression, partial_binding: bool = False
    ):
    ...
    def __call__(
        self, *args, **kwargs
    ) -> complex | ast.Expression | PulseExpression:
        ...
        if not isinstance(expr.body, ast.Constant):
            if self._partial_binding:
                ...
                raise PulseError(
                    f"Parameters {self.params} are not all bound."
                )
    ...
    def _match_ops(opr: ast.AST, opr_dict: dict, *args) -> complex:
        ...
        for op_type, op_func in opr_dict.items():
            if isinstance(opr, op_type):
                return op_func(*args)
        raise PulseError(
            f"Operator {opr.__class__.__name__} is not supported."
        )
        ...
        if node.id in self._math_ops:
            ...
            if not isinstance(_val, ParameterExpression):
                ...
                try:
                    ...
                    raise PulseError(
                        f"Invalid parameter value {node.id} = "
                        f"{self._locals_dict[node.id]} is "
                        "specified."
                    ) from ex
    ...
    def visit_UnaryOp(self, node: ast.UnaryOp) -> ast.UnaryOp | ast.Constant:
        """Evaluate unary operation and return ``ast.Constant`` if bound.
        ...
        if isinstance(node.operand, ast.Constant):
            val = ast.Constant(
                self._match_ops(node.op, self._unary_ops, node.operand.value)
            )
            return ast.copy_location(val, node)
        return node

    def visit_BinOp(self, node: ast.BinOp) -> ast.BinOp | ast.Constant:
        """Evaluate binary operation and return ``ast.Constant`` if bound.
    ...
    def visit_Call(self, node: ast.Call) -> ast.Call | ast.Constant:
        """Evaluate a function call and return ``ast.Constant`` if bound.
        ...
        constant_args: list[ast.Constant] = []
        for arg in node.args:
            if not isinstance(arg, ast.Constant):
                return node
            constant_args.append(arg)

        math_op = self._math_ops.get(func_name)
        if math_op is None or not callable(math_op):
            raise PulseError(f"Function {func_name} is not supported.")

        _args = [arg.value for arg in constant_args]
        _val = math_op(*_args)
        if not _val.imag:
            _val = _val.real
        val = ast.Constant(_val)
...
def parse_string_expr(
    source: str, partial_binding: bool = False
) -> PulseExpression:

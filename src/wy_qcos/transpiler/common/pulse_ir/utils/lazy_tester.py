...
class LazyDependencyManager(abc.ABC):
    ...
    def _is_available(self) -> bool:
        """Subclasses of :class:`LazyDependencyManager` should override this
        method to implement the actual test of availability.  This method
        should return a Boolean, where ``True`` indicates that the dependency
        was available.  This method will only ever be called once.
    ...
    @typing.overload
    def require_in_call(
        self, feature_or_callable: str
    ) -> Callable[[Callable], Callable]: ...

    def require_in_call(self, feature_or_callable):
        """Create a decorator for callables that requires that the dependency
        is available when the decorated function or method is called.
    ...
    @typing.overload
    def require_in_instance(
        self, feature_or_class: str
    ) -> Callable[[type], type]: ...

    def require_in_instance(self, feature_or_class):
        """A class decorator that requires the dependency is available when the
        class is initialized.  This decorator can be used even if the class
        ...
        if isinstance(feature_or_class, str):
            ...
            def decorator(class_):
                wrap_method(
                    class_, "__init__", before=_RequireNow(self, feature)
                )
                return class_
    ...
    def require_now(self, feature: str):
        """Eagerly attempt to import the dependencies in this object, and raise
        an exception if they cannot be imported.
        ...
        raise MissingOptionalLibraryError(
            libname=self._name,
            name=feature,
            pip_install=self._install,
            msg=self._msg,
        )

    @contextlib.contextmanager
    def disable_locally(self):
        """Create a context, during which the value of the dependency manager
        will be ``False``.

        This means that within the context, any calls to this object will
        behave as if the dependency is not available, including raising errors.
        It is valid to call this method whether or not the dependency has
        already been evaluated.  This is most useful in tests.
        """
        previous = self._bool
        self._bool = False
        try:
...
class LazyImportTester(LazyDependencyManager):
    """A lazy dependency tester for importable Python modules.

    Any required objects will only be imported at the point that this object is
    ...
    def __init__(
        ...
        *,
        ...
        if isinstance(name_map_or_modules, dict):
            self._modules = {
                module: tuple(names)
                for module, names in name_map_or_modules.items()
            }
        ...
        super().__init__(
            name=name, callback=callback, install=install, msg=msg
        )
    ...
    def _is_available(self):
        ...
        if failed_modules or failed_names:
            package_description = (
                f"'{self._name}'" if self._name else "optional packages"
            )
            message = (
            ...
            for module, import_error in failed_modules.items():
                message += "".join(
                    f"\n - module '{module}' failed to import with: "
                    f"{import_error!r}"
                )
...
class LazySubprocessTester(LazyDependencyManager):
    ...
    The command will only be run once, at the point that this object is checked

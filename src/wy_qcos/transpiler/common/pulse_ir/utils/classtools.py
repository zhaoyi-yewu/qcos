...
class _lift_to_method:  # pylint: disable=invalid-name
    """A decorator that ensures that an input callable object implements
    ``__get__``.

    It is returned unchanged if so, otherwise it is turned into the default implementation for functions,
    ...
    don't; using this class allows us to do::
        ...
        wrap_method(
            MyClass, "maybe_mutates_arguments", before=print, after=print
        )
...
class _WrappedMethod:
    """Descriptor which calls its two arguments in succession, correctly
    handling instance- and class-method calls.

    It is intended that this class will replace the attribute that ``inner``
    previously was on a class or instance.  When accessed as that attribute,
    this descriptor will behave it is the same function call, but with the
    ``function`` called before or after.
    """
    ...
    def __init__(self, method, before=None, after=None):
        ...
        before = (
            (self._method_decorator(before),) if before is not None else ()
        )
    ...
    def __get__(self, obj, objtype=None):
        ...
        method = (
            self._method.__get__(obj, objtype)
            if self._method_has_get
            else self._method
        )
...
def wrap_method(
    ...
    *,
    ...
    """Wrap the functionality the instance- or class method ``cls.name`` with
    additional behavior ``before`` and ``after``.

...
def deprecate_func(
    *,
    ...
    def decorator(func):
        qualname = (
            func.__qualname__
        )  # For methods, `qualname` includes the class name.
        ...
        add_deprecation_to_docstring(
            wrapper, msg, since=since, pending=pending
        )
        return wrapper
...
def deprecate_arg(
    ...
    *,
    ...
    def decorator(func):
        ...
        deprecated_entity = (
            deprecation_description or f"``{func_name}``'s argument ``{name}``"
        )
        ...
        if hasattr(func, "__original_func_co_varnames"):
            wrapper.__original_func_co_varnames = (
                func.__original_func_co_varnames
            )
        else:
    ...
    def decorator(func):
        ...
        for msg in old_kwarg_to_msg.values():
            add_deprecation_to_docstring(
                ...
                msg,
                since=since,
                pending=issubclass(category, PendingDeprecationWarning),
            )
        return wrapper
...
def _maybe_warn_and_rename_kwarg(
    ...
    *,
    ...
    # positional args.
    arg_names_to_values = {
        name: val for val, name in zip(args, original_func_co_varnames)
    }
    ...
    if new_alias and new_alias in arg_names_to_values:
        raise TypeError(
            f"{func_name} received both {new_alias} and {old_arg_name} (deprecated)."
        )
...
def _write_deprecation_msg(
    *,
    ...
    if pending:
        category: (
            type[DeprecationWarning] | type[PendingDeprecationWarning]

...
HAS_TOQM = _LazyImportTester(
    "qiskit_toqm", name="Qiskit TOQM", install="pip install qiskit-toqm"
)
...
HAS_IPYWIDGETS = _LazyImportTester(
    "ipywidgets", install="pip install ipywidgets"
)
...
HAS_JUPYTER = _LazyImportTester(
    ["jupyter", "nbformat", "nbconvert"], install="pip install jupyter"
)
...
HAS_NLOPT = _LazyImportTester(
    "nlopt", name="NLopt Optimizer", install="pip install nlopt"
)
HAS_PIL = _LazyImportTester(
    "PIL.Image", name="pillow", install="pip install pillow"
)
...
HAS_TWEEDLEDUM = _LazyImportTester(

...
from wy_qcos.transpiler.common.pulse_ir.compatible import exceptions

DEFAULT_FILENAME = os.path.join(
    os.path.expanduser("~"), ".qiskit", "settings.conf"
)
...
class UserConfig:
    ...
    def read_config_file(self):
        ...
        if "default" in self.config_parser.sections():
            ...
            if circuit_drawer:
                if circuit_drawer not in [
                    ...
                    raise exceptions.QiskitUserConfigError(
                        f"{circuit_drawer} is not a valid circuit drawer "
                        "backend. Must be "
            ...
            # Parse state_drawer
            state_drawer = self.config_parser.get(
            ...
            if state_drawer:
                ...
                if state_drawer not in valid_state_drawers:
                    ...
                    raise exceptions.QiskitUserConfigError(
                        f"'{state_drawer}' is not a valid state drawer "
                        "backend. "
            ...
            if circuit_mpl_style:
                if not isinstance(circuit_mpl_style, str):
                    warn(
                        f"{circuit_mpl_style} is not a valid mpl circuit "
                        "style. Must be "
            ...
            try:
                ...
                raise exceptions.QiskitUserConfigError(
                    "Value assigned to circuit_idle_wires is not valid. "
                    f"{str(err)}"
                )
            ...
            if transpile_optimization_level != -1:
                if not 0 <= transpile_optimization_level <= 3:
                    raise exceptions.QiskitUserConfigError(
                        "%s is not a valid optimization level. Must be 0, "
                        "1, 2, or 3."
                    )
...
def set_config(key, value, section=None, file_path=None):
    ...
    if section in [None, "default"]:
        if key not in valid_config:
            raise exceptions.QiskitUserConfigError(
                f"{key} is not a valid user config."
            )

...
from .common import QobjDictField
...
class QasmQobjExperiment:
    ...
    Each instance of this class is used to represent an OpenQASM 2 experiment
    as part of a larger OpenQASM 2 qobj.
    """
    ...
    def __init__(self, config=None, header=None, instructions=None):
        ...
        Args:
            config (QasmQobjExperimentConfig): A config object for the
                experiment.
            header (QasmQobjExperimentHeader): A header object for the
                experiment.
            instructions (list): A list of :class:`QasmQobjInstruction`
                objects.
        """
    ...
    def __repr__(self):
        ...
        return (
            "QasmQobjExperiment("
            f"config={repr(self.config)}, header={repr(self.header)},"
    ...
    def from_dict(cls, data):
        ...
        if "instructions" in data:
            instructions = [
                QasmQobjInstruction.from_dict(inst)
...
class QasmQobjConfig(SimpleNamespace):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            meas_return (str): For measurement level < 2, whether single or
                avg shots are returned.
            ...
            calibrations (QasmExperimentCalibrations): Information required
                for Pulse gates.
            rep_delay (float): Delay between programs in sec. Only supported
                on certain backends
                (``backend.configuration().dynamic_reprate_enabled`` ). Must
                be from the range supplied by the backend
                (``backend.configuration().rep_delay_range``). Default
                is ``backend.configuration().default_rep_delay``.
            qubit_lo_freq (list): List of frequencies for the qubit driver
                LO's in GHz.
            meas_lo_freq (list): List of frequencies for the measurement
                driver LO's in GHz.
    ...
    def to_dict(self):
        """Return a dictionary representation of the OpenQASM 2 Qobj config.
    ...
    def from_dict(cls, data):
        ...
        if "calibrations" in data:
            ...
            data["calibrations"] = QasmExperimentCalibrations.from_dict(
                calibrations
            )
...
class QasmQobjExperimentConfig(QobjDictField):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """Instantiate a QasmQobjExperimentConfig object.

        Args:
            calibrations (QasmExperimentCalibrations): Information required
                for Pulse gates.
            qubit_lo_freq (List[float]): List of qubit LO frequencies in GHz.
            meas_lo_freq (List[float]): List of meas readout LO frequencies
                in GHz.
            kwargs: Additional free form key value fields to add to the
                configuration.
        """
...
class QasmExperimentCalibrations:
    ...
    The gates attribute contains a list of GateCalibrations.
    """
    ...
    def __init__(self, gates):
        ...
        Args:
            gates (list(GateCalibration)): Gate calibrations.
        """
        self.gates = gates
    ...
    def from_dict(cls, data):
        ...
        Args:
            data (dict): A dictionary representing the
                QasmExperimentCalibrations to create. It will be in the same
                format as output by :func:`to_dict`.

        Returns:
            QasmExperimentCalibrations: The QasmExperimentCalibrations from
...
class GateCalibration:
    """Describe a unique gate calibration.
    ...
    def __init__(self, name, qubits, params, instructions):
        ...
        Instructions may reference waveforms which should be made available in
        the pulse_library.
    ...
    def from_dict(cls, data):
        ...
        Args:
            data (dict): A dictionary representing the GateCalibration to
        ...
        data["instructions"] = [
            PulseQobjInstruction.from_dict(x) for x in instructions
        ]
        return cls(**data)
...
class QasmQobj:
    ...
    def __init__(
        self, qobj_id=None, config=None, experiments=None, header=None
    ):
        """Instantiate a new OpenQASM 2 Qobj Object.

        Each OpenQASM 2 Qobj object is used to represent a single payload that
        will be passed to a Qiskit provider. It mirrors the published Qobj
        specification for OpenQASM experiments.
    ...
    def to_dict(self):
        ...
        for use with IBM systems, you can leverage a json encoder that
        converts these as expected. For example:

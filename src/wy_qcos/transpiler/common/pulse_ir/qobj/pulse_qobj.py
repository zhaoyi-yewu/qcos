...
class PulseQobjInstruction:
    ...
    def from_dict(cls, data):
        ...
        # To avoid deepcopy and avoid mutating the source object, create a
        # new dict here.
        in_data = {}
        for key, value in data.items():
            ...
            if key == "parameters":
                ...
                if "amp" in formatted_value:
                    formatted_value["amp"] = _to_complex(
                        formatted_value["amp"]
                    )
        ...
        **kwargs,
        ...
        Args:
            ...
            memory_slot_size (int): Size of each memory slot if the output is
                Level 0.
            rep_time (int): Time per program execution in sec. Must be from
                the list provided by the backend
                (``backend.configuration().rep_times``). Defaults to the first
                entry in ``backend.configuration().rep_times``.
            rep_delay (float): Delay between programs in sec. Only supported
                on certain backends
                (``backend.configuration().dynamic_reprate_enabled`` ). If
                supported, ``rep_delay`` will be used instead of ``rep_time``
                and must be from the range supplied by the backend
                (``backend.configuration().rep_delay_range``). Default is
...
class PulseQobjExperiment:
    ...
    def __init__(self, instructions, config=None, header=None):
        ...
        Args:
            config (PulseQobjExperimentConfig): A config object for the
                experiment.
            header (PulseQobjExperimentHeader): A header object for the
                experiment.
            instructions (list): A list of :class:`PulseQobjInstruction`
                objects.
        """

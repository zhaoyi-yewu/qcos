...
"""Convert pulse instructions to and from PulseQobjInstruction."""
...
from wy_qcos.transpiler.common.pulse_ir.pulse.exceptions import QiskitError
from wy_qcos.transpiler.common.pulse_ir.pulse.parser import parse_string_expr
from wy_qcos.transpiler.common.pulse_ir.pulse.schedule import Schedule
from ..common import QOBJ_DEPRECATION_MSG
from .. import QobjMeasurementOption, PulseLibraryItem, PulseQobjInstruction
...
class ParametricPulseShapes(Enum):
    ...
    The enum name is the transport layer name for pulse shapes, the value is
    its mapping to the OpenPulse Command in Qiskit.
    """
    ...
    @classmethod
    def to_type(cls, name: str) -> Callable[..., library.SymbolicPulse]:
...
class InstructionToQobjConverter:
    ...
    def __init__(
        self,
        qobj_model: Callable[..., PulseQobjInstruction],
        **run_config,
    ):
    ...
    def __call__(
        self,
        shift: int,
        instruction: instructions.Instruction | list[instructions.Acquire],
    ) -> PulseQobjInstruction:
        """Convert Qiskit in-memory representation to Qobj instruction.

        Args:
            shift: Time offset in dt units.
        ...
        Raises:
            QiskitError: When a non-Acquire instruction list is provided.
        """
        if isinstance(instruction, list):
            if all(
                isinstance(inst, instructions.Acquire) for inst in instruction
            ):
    ...
    def _convert_play(
        ...
        if isinstance(instruction.pulse, library.SymbolicPulse):
            ...
            if "amp" in params and "angle" in params:
                params["amp"] = complex(
                    params["amp"] * np.exp(1j * params["angle"])
                )
                del params["angle"]
    ...
    def _convert_bundled_acquire(
        ...
        Raises:
            ...
            QiskitError: When discriminator or kernel is missing in part of
                instructions.
        """
        meas_level = self._run_config.get("meas_level", 2)
        ...
        for instruction in instruction_bundle:
            ...
            if instruction.start_time != t0:
                raise QiskitError(
                    "The supplied acquire instructions have different "
                    "starting times. Something has gone wrong calling this "
                    "code. Please report this issue."
                )

            if instruction.duration != duration:
                raise QiskitError(
                    "Acquire instructions beginning at the same time must "
                    "have same duration."
                )
        ...
        if discriminators:
            ...
            if num_discriminators == len(qubits) or num_discriminators == 1:
                ...
                raise QiskitError(
                    "A discriminator must be supplied for every acquisition "
                    "or a single "
        ...
        if kernels:
            ...
            if num_kernels == len(qubits) or num_kernels == 1:
                ...
                raise QiskitError(
                    "A kernel must be supplied for every acquisition or a "
                    "single "
...
class QobjToInstructionConverter:
    ...
    This converter converts data from transfer layer into the in-memory
    representation of the front-end of Qiskit Pulse.
    ...
    where ``NewInstruction`` must be a subclass of
    :class:`~wy_qcos.pulse.instructions.Instruction`.
    """
    ...
    def __init__(
        self,
        pulse_library: list[PulseLibraryItem] | None = None,
        **run_config,
    ):
        """Create new converter.
    ...
    def _get_sequences(
        ...
        """Iterate over pulse instructions without creating a Schedule.

        .. note::

            This is internal fast-path function, and callers other than this
            converter class might directly use this method to generate schedule
            from multiple Qobj instructions. Because __call__ always returns a
            schedule with the time offset parsed instruction, composing
            multiple Qobj instructions to create a gate schedule is somewhat
            inefficient due to composing overhead of schedules. Directly
            combining instructions with this method is much performant.
    ...
    def disassemble_value(
        value_expr: float | str,
    ) -> float | ParameterExpression:
        """A helper function to format instruction operand.

        If parameter in string representation is specified, this method parses
        the input string and generates Qiskit ParameterExpression object.

        Args:
            value_expr: Operand value in Qobj.

        Returns:
            Parsed operand value. ParameterExpression object is returned if
            value is not number.
        """
    ...
    def _convert_acquire(
        ...
        if any(
            ...
            warnings.warn(
                "Can currently only support one discriminator per acquire. "
                "Defaulting "
        ...
        kernels = (
            instruction.kernels if hasattr(instruction, "kernels") else None
        )
        if not isinstance(kernels, list):
            kernels = [kernels]
        if any(kernels[0] != kernels[i] for i in range(len(kernels))):
            warnings.warn(
                "Can currently only support one kernel per acquire. "
                "Defaulting to first "
                "kernel entry."
            )
        kernel = kernels[0]
    ...
    def _convert_parametric_pulse(
        ...
        .. note::
            ...
            If parametric pulse label is not provided by the backend, this
            method naively generates a pulse name based on the pulse shape and
            bound parameters. This pulse name is formatted to, for example,
            `gaussian_a4e3`, here the last four digits are a part of the hash
            string generated based on the pulse shape and the parameters.
            Because we are using a truncated hash for readability,
            there may be a small risk of pulse name collision with other
            pulses.
            Basically the parametric pulse name is used just for visualization
            purpose and the pulse module should not have dependency on the
            parametric pulse names.
        ...
        try:
            ...
            sorted_params = sorted(
                instruction.parameters.items(), key=lambda x: x[0]
            )
            base_str = f"{instruction.pulse_shape}_{str(sorted_params)}"
            short_pulse_id = hashlib.md5(base_str.encode("utf-8")).hexdigest()[
                :4
            ]

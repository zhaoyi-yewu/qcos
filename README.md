...
| | 功能 |
|:--------------|:------------------------------------|
| <nobr>交互方式</nobr> | 命令行、API、SDK、GUI（规划） |
| <nobr>系统服务</nobr> | 配置管理、日志管理、用户管理（规划）、权限管理（规划）、监控管理（规划） |
| <nobr>设备管理</nobr> | 校准操作（规划）、设备配置/查询 |
| <nobr>作业管理</nobr> | 作业提交、取消、删除、状态查询、结果查询 |
| <nobr>系统引擎</nobr> | QASM解析、逻辑门分解、量子比特映射、编译优化、线路聚合、线路拆分（规划）、量子纠错QEC（规划） |
| <nobr>驱动插件</nobr> | dummy测试驱动、光量子、中性原子、超导、离子阱等 |

...
class MCTree(DiGraph):
    ...
    Args:
        ...
        select_mode：选择步骤采用的策略，默认为Kocsis and Szepesvári method
        mode_BP：反向传播的方式，目前仅支持globalscore
        mode_decision：选择子节点的依据，默认为globalscore，即选择global_score最大的子节点
        objective：优化的策略，是减小size还是depth
        root_node：根节点
    """
    ...
    def add_node_mcts(self, father_node, added_swap=None,
                       remote_exe_node=None):
        '''
        ...
        if father_node is None:
            ...
            if self.objective == 'no_swap' and cir.num_remain_nodes > 0:
                raise ValueError(
                    "Fail to find a mapping requiring no swaps."
                )
        else:
            ...
            if remote_exe_node is not None:
                remote_cxs, exe_gates = (
                    cir.execute_gate_remote(remote_exe_node)
                )
                num_add_gates_new = (
    ...
    def add_depth(self, node):
        ...
        if father_node is None:
            ...
            depth_add = max(
                depth_after - np.max(depth_phy_qubits_pre), 0
            )
            #print(depth_add)
        ...
        for node_DG in new_exe_nodes_DG:
            for _, qubits, _ in self.DG.nodes[node_DG]['gates']:
                ...
                if num_q == 1:
                    ...
                    depth_phy_qubits[q_phy] += 1
                if num_q == 2:
                    q_phy0 = log_to_phy[qubits[0]]
                    q_phy1 = log_to_phy[qubits[1]]
                    depth0, depth1 = (
    ...
    def expansion(self, node):
        ...
        swaps, swap_scores, swap_scores_front = (
            self.nodes[node]['circuit'].pertinent_swaps(
    ...
    def fallback(self):
        ...
        while (self.nodes[start_node]['local_score'] == 0 and
        ...
        path = self.shortest_path_AG[chosen_CX_phy[0]][
            chosen_CX_phy[1]].copy()
    ...
    def decision(self, mode_decision=None):
        ...
        if display_state:
            print(f'\r{self.nodes[self.root_node]["num_remain_gates"]} '
                  f'gates unfinished', end='')
    ...
    def print_son_attrs(self, father_node, names_son, names_father=[]):
        if (not isinstance(names_son, list) and
                not isinstance(names_son, tuple)):
            raise ValueError(
                f'names argument must be list or tuple, but it is '
                f'{type(names_son)}'
            )
        if (not isinstance(names_father, list) and
                not isinstance(names_father, tuple)):
            raise ValueError(
                f'names argument must be list or tuple, but it is '
                f'{type(names_father)}'
            )

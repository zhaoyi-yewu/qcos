...
class Gate:
    ...
    def __init__(self, name, targets=None, arg_value=None, gate_type=GateType.SINGLE_QUBIT_GATE.value, hermitian=True) \
            -> None:
        """
        Args:
            name (_type_): 门名称
        ...
        self.arg_value = arg_value if arg_value is not None else []
    ...
    def decompose(self):
        ...
        其中based_gates为一个三元组（name, targets, exps）,name表示门名称，targets为作用比特下标列表(从0开始)，exps为参数对应的表达式，以字符串的形式表示，表达式中的操作数可为param中定义的形式化参数以及常量，常量中π可用pi表示
        ...
        try:
            ...
            if len(params_list) != len(need_args):
                raise ValueError(f"{self.name} gate need {len(need_args)} params but give {len(params_list)}")
            params = dict(zip(params_list, need_args))
            params['pi'] = np.pi
            decomposed_gates = custom_gate.get('gates', [])
            if len(decomposed_gates) == 0: return [self]
            gates = []
    ...
    def __repr__(self):
        return f"{type(self).__name__}(targets={self.targets}, arg_value={self.arg_value})"
...
class X(Gate):
    ...
    def default_decompose(self):
        return [RX(targets=self.targets, arg_value=np.pi)]
...
class Y(Gate):
    ...
    def default_decompose(self):
        return [RY(targets=self.targets, arg_value=np.pi)]
...
class SDG(Gate):
    """
    反相位门类, 是S门的共轭转置，对量子态的|1⟩分量施加一个相位变换，使得|1⟩变为-i∣1⟩，而|0⟩分量保持不变
...
class T(Gate):
    """
    T门，用于实现较小的相位旋转。T门的作用是对量子态的|1⟩分量施加一个相位变换，使得|1⟩变为e^iπ/4∣1⟩，而|0⟩分量保持不变
    ...
    def default_decompose(self):
        return [RZ(targets=self.targets, arg_value=np.pi / 4)]


class TDG(Gate):
    """
    TDG门，T门的共轭转置, 作用是对量子态的|1⟩分量施加一个相位变换，使得|1⟩变为e^-iπ/4∣1⟩，而|0⟩分量保持不变
...
class CZ(Gate):
    """
    受控Z门或Controlled-Z门, 在控制量子比特为|1⟩时，对目标量子比特应用一个Z门（Pauli-Z门）将目标量子比特的相位翻转
    CZ门在Bloch球中对应于绕Z轴旋转π角度, 仅当控制量子比特为|1⟩时
    """

    def __init__(self, targets=None, arg_value=None, gate_type=GateType.DOUBLE_QUBIT_GATE.value) -> None:
...
class CX(Gate):
    """
    受控非门或Controlled-X门, 当控制位处于|1⟩状态时，将目标位翻转（即|0⟩变为|1⟩，|1⟩变为|0⟩）。如果控制位处于|0⟩状态，则目标位保持不变
    对应于经典比特的XOR（异或）操作
    """
...
class CY(Gate):
    """
    受控Y门或Controlled-Y门, 在控制量子比特为|1⟩时，对目标量子比特应用一个Y门（Pauli-Y门）将目标量子比特绕Y轴旋转π角度
...
class CRX(Gate):
    ...
    def __init__(self, targets=None, arg_value=None, gate_type=GateType.DOUBLE_QUBIT_GATE.value) -> None:
        super().__init__('crx', targets, arg_value, gate_type, hermitian=False)

    def default_decompose(self):
        gates = []
        gates += H([self.targets[1]]).decompose()
        gates.append(CX(self.targets))
        gates.append(RZ(targets=[self.targets[1]], arg_value=-self.arg_value[0] / 2))
        gates.append(CX(self.targets))
        gates.append(RZ(targets=[self.targets[1]], arg_value=self.arg_value[0] / 2))
        gates += H([self.targets[1]]).decompose()
        return gates
...
class CRY(Gate):
    ...
    def __init__(self, targets=None, arg_value=None, gate_type=GateType.DOUBLE_QUBIT_GATE.value) -> None:
        super().__init__('cry', targets, arg_value, gate_type, hermitian=False)

    def default_decompose(self):
        gates = []
        gates.append(CX(self.targets))
        gates.append(RY(targets=[self.targets[1]], arg_value=-self.arg_value[0] / 2))
        gates.append(CX(self.targets))
        gates.append(RY(targets=[self.targets[1]], arg_value=self.arg_value[0] / 2))
        return gates
...
class CRZ(Gate):
    ...
    def __init__(self, targets=None, arg_value=None, gate_type=GateType.DOUBLE_QUBIT_GATE.value) -> None:
        super().__init__('crz', targets, arg_value, gate_type, hermitian=False)
...
class CCX(Gate):
    ...
    def __init__(self, targets=None, arg_value=None, gate_type=GateType.TRIPLE_QUBIT_GATE.value) -> None:
        super().__init__('ccz', targets, arg_value, gate_type)
...
class U1(Gate):
    ...
    def default_decompose(self):
        return [RZ(self.targets, self.arg_value)]
...
class SYNC(Gate):
    ...
    def __init__(self, targets=None, arg_value=None, gate_type=GateType.SYNC_GATE.value) -> None:
        super().__init__('sync', targets, arg_value, gate_type, hermitian=False)
...
class MEASURE(Gate):
    ...
    def __init__(self, targets=None, arg_value=None, gate_type=GateType.MEASURE_GATE.value) -> None:
        super().__init__('measure', targets, arg_value, gate_type, hermitian=False)
...
class MOV(Gate):
    ...
    def __init__(self, targets=None, arg_value=None, gate_type=GateType.MOVE_GATE.value) -> None:
        super().__init__('mov', targets, arg_value, gate_type, hermitian=False)
...
def create_gate(name, targets=None, arg_value=None, allow_undefined=False):
    ...
    Args:
        ...
        allow_undefined (bool, optional): 是否允许自定义的门（线路切割可视化添加切割点时设置为True）. Defaults to False.
    return:
        Gate: 门名称对应的量子比特门实例
    """
    if name == 'h':
        ...
        if allow_undefined: return Gate(name, targets=targets, arg_value=arg_value)
...
def optimizer(ir: list):
    ...
    for gate in ir:
        ...
        if optimize_gates:
            pre_gate = optimize_gates[-1]
            if gate.name == pre_gate.name and gate.targets == pre_gate.targets:
                if gate.hermitian:
                    optimize_gates.pop(-1)
                    continue
                if gate.name in ['rx', 'ry', 'rz', 'crx', 'cry', 'cry', 'u1']:
                    optimize_gates[-1].arg_value[0] += gate.arg_value[0]
                    optimize_gates[-1].arg_value[0] %= (4 * np.pi)
                    if optimize_gates[-1].arg_value[0] < 1e-5:
                        optimize_gates.pop(-1)
                    continue
            elif gate.name == 's' and pre_gate.name == 'sdg':
                optimize_gates.pop(-1)
                continue
            elif gate.name == 't' and pre_gate.name == 'tdg':
                optimize_gates.pop(-1)
                continue

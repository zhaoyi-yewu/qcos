...
from qcos.transpiler.cmss.wirecut.cut_wire import (
    CutWire,
...
from qcos.transpiler.cmss.common.gate_operation import H, CX
...
class TestCutWire(unittest.TestCase):
    ...
    @patch(
        "qcos.transpiler.cmss.wirecut.cut_wire.open",
        side_effect=lambda *args, **kwargs: MagicMock(),
    )
    @patch("qcos.transpiler.cmss.wirecut.mip_model.MIPModel")
    def test_cutwire_init_success(self, mock_mip_model, mock_open):
        ...
        with patch("qcos.transpiler.cmss.wirecut.cut_wire.DAG") as mock_dag:
            ...
            with patch(
                "qcos.transpiler.cmss.wirecut.cut_wire.Cut"
            ) as mock_cut:
                qc = QuantumCircuit(4)
                qc.append(H([0]))
                qc.append(CX([0, 1]))
                ...
                with patch(
                    "qcos.transpiler.cmss.wirecut.cut_wire.Prepare_data"
                ):
    ...
    @patch("qcos.transpiler.cmss.wirecut.cut_wire.CutWire")
    @patch(
        "qcos.transpiler.cmss.wirecut.cut_wire.open",
        side_effect=lambda *args, **kwargs: MagicMock(),
    )
    ...
    def test_generate_all_variant_subcircuits_for_execute_default_max_cuts(
        ...
        with patch(
            "qcos.transpiler.cmss.wirecut.cut_wire.CutWire"
        ) as mock_cutwire:
    ...
    @patch("qcos.transpiler.cmss.wirecut.cut_wire.DD")
    @patch("qcos.transpiler.cmss.wirecut.cut_wire.reconstruct_prob_from_bins")
    ...
    def test_reconstruct_probability_distribution_wire_cut_edge_cases(self):
        ...
        with (
            patch("qcos.transpiler.cmss.wirecut.cut_wire.DD") as mock_dd,
            patch(
                "qcos.transpiler.cmss.wirecut.cut_wire."

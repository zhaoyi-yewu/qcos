...
void SymbolicOperation::checkSymbolicUgate() {
  ...
  if (type == otP) {
    ...
    if (!isSymbolicParameter(0) && !isSymbolicParameter(1) &&
      ...
      type = parseU3(symbolicParameter[0].value(),
                     symbolicParameter[1].value(), parameter[2]);
    }
  }
...
[[nodiscard]] fp SymbolicOperation::getInstantiation(
...
SymbolicOperation::SymbolicOperation(const Control control,
                                     const Targets& targ, const OpType g,
                                     const std::vector<SymbolOrNumber>& params)
...
[[noreturn]] void SymbolicOperation::dumpOpenQASM(
    ...
    [[maybe_unused]] const RegisterNames& creg, [[maybe_unused]] size_t indent,

...
void NonUnitaryOperation::MEASURE_dumpOriginIR(std::ostream& of,
                                               const RegisterNames& qreg,
                                               const RegisterNames& creg,
                                               size_t indent) const {
  auto classicsIt = classics.cbegin();
...
void NonUnitaryOperation::RESET_dumpOriginIR(std::ostream& of,
                                             const RegisterNames& qreg,
                                             const RegisterNames& creg,
                                             size_t indent) const {
  auto classicsIt = classics.cbegin();
...
void NonUnitaryOperation::BARRIER_dumpOriginIR(std::ostream& of,
                                               const RegisterNames& qreg,
                                               const RegisterNames& creg,
                                               size_t indent) const {
  auto classicsIt = classics.cbegin();
...
void NonUnitaryOperation::dumpOriginIR(std::ostream& of,
  ...
  for (const auto& q : targets) {
      ...
      ss << toString(type) << " " << qreg[q].second;
      std::cout <<
  "non_unitary_operation.cpp��NonUnitaryOperation::dumpOriginIR>>3>>1ss:" <<
  ss.str() << std::endl; if (type == otMeasure) { of << "," <<
  creg[*classicsIt].second; ss << "," << creg[*classicsIt].second; std::cout <<
  "non_unitary_operation.cpp��NonUnitaryOperation::dumpOriginIR>>3>>2ss:" <<
  ss.str() << std::endl;
          ++classicsIt;
      }
      of << "\n";
      ss << "\n";
      std::cout <<
  "non_unitary_operation.cpp��NonUnitaryOperation::dumpOriginIR>>3>>3ss:"<<ss.str()
  << std::endl;
  }
  */
  if (type == otMeasure) {

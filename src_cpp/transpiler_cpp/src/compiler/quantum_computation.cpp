  ...
  for (const auto& opIt : ops) {
    if (const auto* const op = dynamic_cast<NonUnitaryOperation*>(opIt.get());
      ...
      for (const auto& q : op->getTargets()) {
        ...
        // only the first measurement of a qubit is used to determine the
        // output permutation
...
void QuantumComputation::addClassicalRegister(std::size_t nc,
  ...
  if (cregs.count(regName) != 0) {
    throw QFRException(
        "[addClassicalRegister] Augmenting existing classical "
...
std::pair<QBit, std::optional<QBit>> QuantumComputation::removeQubit(
...
// adds j-th physical qubit as ancilla to the end of reg or creates the
...
void QuantumComputation::dump(std::ostream&& of, Format format) {
  switch (format) {
    ...
    default:
      throw QFRException(
          "[dump] Format not recognized/supported for dumping.");
  }
}
...
std::string QuantumComputation::getQubitRegister(
...
std::string QuantumComputation::getClassicalRegister(
...
std::ostream& QuantumComputation::printPermutation(
    const Permutation& permutation, std::ostream& os) {
...
QuantumComputation::containsLogicalQubit(const QBit logicalQubitIndex) const {
  if (const auto it =
          std::find_if(initialLayout.cbegin(), initialLayout.cend(),
...
void QuantumComputation::measure(
  ...
  if (const auto cRegister = cregs.find(registerBit.first);
    ...
    emplace_back<NonUnitaryOperation>(
        qubit, cRegister->second.first + registerBit.second);

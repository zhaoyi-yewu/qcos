    ...
    if (auto powModifier =
      ...
      if (auto evaluatedArg = visit(powModifier->expression)) {
        powModifier->expression = evaluatedArg->toExpr();
               ...
               ctrlModifier->expression != nullptr) {
...
std::optional<ConstEvalValue> ConstEvalPass::visitBinaryExpression(
  ...
  // If we are operating on two types with different width, coerce to the
  // wider.
  size_t const width = std::max(lhsVal->width, rhsVal->width);

  switch (lhsVal->type) {
    case ConstEvalValue::Type::ConstInt:
      return evalIntExpression(binaryExpression->op,
                               std::get<0>(lhsVal->value),
    ...
    case ConstEvalValue::Type::ConstFloat:
      return evalFloatExpression(binaryExpression->op,
                                 std::get<1>(lhsVal->value),
                                 std::get<1>(rhsVal->value));
    case ConstEvalValue::Type::ConstBool:
      return evalBoolExpression(binaryExpression->op,
                                std::get<2>(lhsVal->value),

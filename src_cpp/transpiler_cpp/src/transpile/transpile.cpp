...
TranspileResult transpile_na_from_ir(
  ...
  // Step 1: Optimize #1 — lightweight pre-mapping optimization (opt_level
  // capped at 1).
  auto opt1_start = clock::now();
  std::set<std::string> basis_set(supp_basis_gates.begin(),
                                  supp_basis_gates.end());
  auto optimized_ops =
      optimize(ir_ops, std::min(1, opt_level), false, {}, 0);
  t.opt_time1 =
      std::chrono::duration<double>(clock::now() - opt1_start).count();
  ...
  // Step 3: Get decompose rules — build the decomposition rule table from the
  // basis gates.
  // Neutral-atom routing does not insert SWAPs and the basis gate set may
  // lack a two-qubit gate to decompose SWAP, so SWAP decomposition is
  // skipped here (see build_full_decomposition_table).
  ...
  // Step 4: NA mapping — dispatch by na_mapping_type (default: NADefaultRoute),
  // inserting MOVE to shuttle atoms so two-qubit gates act on adjacent sites.
  auto map_start = clock::now();
  auto routed_ops = na_mapping(decomposed_ops, qpu_config, num_qubits,
                               /*na_support_move=*/true, na_mapping_type,
  ...
  // Step 5: Apply decompose rules — replace routed gates with basis gates per
  // the rules.
  auto apply_start = clock::now();
  auto decomposed_circuit =
      decomposer.apply_decompose_rules(routed_ops, decompose_table);

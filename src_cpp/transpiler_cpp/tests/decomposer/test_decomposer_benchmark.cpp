...
void ValidateTargetBasis(
  ...
  for (const auto& op : circuit) {
    ...
    EXPECT_TRUE(target_basis.count(op->name))
        << "Unexpected gate found after decomposition: "
        << op->name;
  }
}
...
TEST_F(QasmDecomposerTest, ParseAndDecomposeBenchmarkCircuit) {
  ...
  const std::vector<std::string> target_basis = {
      "rx",
      "ry",
      "rz",
      "cx"
  ...
  auto gate_names =
      BuildGateNameList(operations);

  auto decompose_start =
      std::chrono::high_resolution_clock::now();

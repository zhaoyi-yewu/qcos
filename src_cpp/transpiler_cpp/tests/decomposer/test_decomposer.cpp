 ...
 */
void ValidateGateIR(const BaseOperation* op, const std::string& expected_name,
                    const std::vector<int>& expected_targets,
                    int expected_qubit_count, bool expect_two_qubit) {
 ...
 */
void ValidateIREquals(const std::vector<std::shared_ptr<BaseOperation>>& lhs,
                      const std::vector<std::shared_ptr<BaseOperation>>& rhs) {
...
TEST(DecomposerCppTest, DecomposeBasisOnly) {
  ...
  ValidateGateIR(result[0].get(), "rx", {0}, 1, false);

  ValidateGateIR(result[1].get(), "ry", {0}, 1, false);
...
TEST(DecomposerCppTest, DecomposeHGate) {
  ...
  ValidateGateIR(result[0].get(), "ry", {0}, 1, false);

  ValidateGateIR(result[1].get(), "rx", {0}, 1, false);
}
...
TEST(DecomposerCppTest, DecomposeCYGate) {
  ...
  // rz(-pi/2)
  ValidateGateIR(result[0].get(), "rz", {1}, 1, false);
  ...
  // cx
  ValidateGateIR(result[1].get(), "cx", {0, 1}, 2, true);

  // rz(pi/2)
  ValidateGateIR(result[2].get(), "rz", {1}, 1, false);
...
TEST(DecomposerCppTest, DecomposeCU3Gate) {
  ...
  source.push_back(create_gate("cu3", {0, 1}, {M_PI, M_PI, M_PI}));
  ...
  const std::vector<std::string> expected_names = {
      "rz", "rz", "cx", "rz", "rx", "rz", "rx",
      "rz", "cx", "rz", "rx", "rz", "rx", "rz"};

  for (size_t i = 0; i < expected_names.size(); ++i) {
    EXPECT_EQ(result[i]->name, expected_names[i]);
  }

  // Validate CX locations
  ValidateGateIR(result[2].get(), "cx", {0, 1}, 2, true);

  ValidateGateIR(result[8].get(), "cx", {0, 1}, 2, true);
}
...
TEST(DecomposerCppTest, DecomposeCUGate) {
  ...
  source.push_back(create_gate("cu", {0, 1}, {M_PI, M_PI, M_PI, M_PI}));

...
class CompoundOperation final : public Operation {
 ...
 public:
   ...
   * @details This transfers ownership of the operations from the other
   * compound operation to this one. The other compound operation will be empty
  ...
  std::vector<std::unique_ptr<Operation>>::iterator erase(
      std::vector<std::unique_ptr<Operation>>::const_iterator pos) {
    return ops.erase(pos);
  }
  std::vector<std::unique_ptr<Operation>>::iterator erase(
      std::vector<std::unique_ptr<Operation>>::const_iterator first,
  ...
  template <class T, class... Args>
  std::vector<std::unique_ptr<Operation>>::iterator insert(
      std::vector<std::unique_ptr<Operation>>::const_iterator iterator,
      Args&&... args) {
...
namespace std {
template <>
struct hash<qc::CompoundOperation> {

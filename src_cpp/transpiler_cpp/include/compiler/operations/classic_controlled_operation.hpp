 ...
 public:
  ...
  ClassicControlledOperation(std::unique_ptr<qc::Operation>& operation,
      ...
      : op(std::move(operation)),
        controlRegister(std::move(controlReg)),
  ...
  ClassicControlledOperation& operator=(
...
namespace std {
template <>
struct hash<qc::ClassicControlledOperation> {
  std::size_t operator()(
      qc::ClassicControlledOperation const& ccop) const noexcept {

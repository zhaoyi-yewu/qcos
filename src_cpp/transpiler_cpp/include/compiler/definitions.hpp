 ...
 */
[[nodiscard]] constexpr std::size_t combineHash(
    const std::size_t lhs, const std::size_t rhs) noexcept {
 ...
 */
constexpr void hashCombine(std::size_t& hash,
                           const std::size_t with) noexcept {
...
static std::map<std::string, std::function<double(double, double)>>
    _binary_operation = {
...
class Exp {
 public:
  ...
  Exp(std::shared_ptr<Exp> left_exp_ptr, std::shared_ptr<Exp> right_exp_ptr,
      std::string op) {

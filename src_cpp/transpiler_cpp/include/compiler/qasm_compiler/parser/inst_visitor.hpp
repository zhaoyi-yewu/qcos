...
class InstVisitor {
 public:
  virtual void visitGateStatement(
      std::shared_ptr<GateDeclaration> gateStatement) = 0;
  ...
  virtual void visitInitialLayout(
      std::shared_ptr<InitialLayout> initialLayout) = 0;
  ...
  virtual void visitBarrierStatement(
      std::shared_ptr<BarrierStatement> barrierStatement) = 0;
  virtual void visitResetStatement(
      std::shared_ptr<ResetStatement> resetStatement) = 0;
...
class DefaultInstVisitor : public InstVisitor {
 public:
  ...
  void visitIfStatement(
      std::shared_ptr<IfStatement> /*ifStatement*/) override {}
};
...
class ExpressionVisitor {
 public:
  virtual T visitBinaryExpression(
      std::shared_ptr<BinaryExpression> binaryExpression) = 0;
  virtual T visitUnaryExpression(
      std::shared_ptr<UnaryExpression> unaryExpression) = 0;
  ...
  virtual T visitIdentifierList(
      std::shared_ptr<IdentifierList> identifierList) = 0;
...
class TypeVisitor {
 public:
  ...
  virtual std::shared_ptr<ResolvedType> visitDesignatedType(
      DesignatedType<T>* designatedType) = 0;
  virtual std::shared_ptr<ResolvedType> visitUnsizedType(
      UnsizedType<T>* unsizedType) = 0;
  virtual std::shared_ptr<ResolvedType> visitArrayType(
      ArrayType<T>* arrayType) = 0;
};
}  // namespace qasm

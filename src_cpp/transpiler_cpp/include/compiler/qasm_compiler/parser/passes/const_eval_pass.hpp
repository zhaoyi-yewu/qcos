...
#include "compiler/qasm_compiler/parser/passes/compiler_pass.hpp"
...
struct ConstEvalValue {
  ...
  explicit ConstEvalValue(int64_t val, const bool isSigned,
                          const size_t w = 64)
...
class ConstEvalPass : public CompilerPass,
  ...
  template <typename T>
  static int64_t castToWidth(int64_t value) {
  ...
  static ConstEvalValue evalIntExpression(BinaryExpression::Op op, int64_t lhs,
                                          ...
                                          bool isSigned);
  static ConstEvalValue evalFloatExpression(BinaryExpression::Op op,
 ...
 public:
  ...
  std::optional<ConstEvalValue> visitConstantExpression(
      std::shared_ptr<Constant> constant) override;
  std::optional<ConstEvalValue> visitIdentifierExpression(
      std::shared_ptr<IdentifierExpression> identifierExpression) override;
  std::optional<ConstEvalValue> visitIdentifierList(
      std::shared_ptr<IdentifierList> identifierList) override;
  ...
  std::shared_ptr<ResolvedType> visitArrayType(
      ArrayType<std::shared_ptr<Expression>>* arrayType) override;
};
}  // namespace qasm::const_eval
